...
| [scripts/stl-device-type-survey.ps1](scripts/stl-device-type-survey.ps1) | Round 8.5 | STL 设备类型调研（三样本）：CBM→DEV→PHM→STL 反查设备类型 |
| [scripts/mod-device-type-survey.ps1](scripts/mod-device-type-survey.ps1) | Round 8.6 | MOD 设备类型对比调研（三样本）：CBM→DEV→PHM→MOD 反查设备类型 |
...
**脚本**：[scripts/stl-device-type-survey.ps1](scripts/stl-device-type-survey.ps1)
...
**脚本**：[scripts/mod-device-type-survey.ps1](scripts/mod-device-type-survey.ps1)
...
| 35 | 变电 STL 设备类型 | 100% SecondaryCabinet（二次设备柜：通信柜/保护测控柜/电能表柜） | 出现一次设备 STL 需详细分析 |
| 36 | 线路 STL 设备类型 | 100% 金具（INSULATOR/SPACER/DAMPER），不渲染塔本体 | 出现塔本体 STL 需详细分析 |
| 37 | 变电 MOD 设备类型 | 一次电气设备（电容器/GIS/接地变/避雷器等，14 种 primitive） | 出现二次柜主导 primitive 需记录 |
| 38 | 线路 MOD 设备类型 | 4 类文本格式族与设备 100% 单射对应（HNUM→TOWER、POINT_LINE→CROSS、SECTION_KV→BASE、KEY_VALUE→WIRE+BASE） | 出现跨 kind 混用需详细分析 |
