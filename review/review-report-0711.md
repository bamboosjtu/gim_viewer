...
评审范围：TS 前端（~80 文件，17292 行）+ Rust 后端（3 文件，3668 行）。仅分析，未修改代码。
...
| 维度 | 评级 | 一句话 |
|---|---|---|
| 架构分层 | B+ | 方向干净，viewer↔ui 边界渗漏 |
| 代码质量 | B | 核心 IR 优秀，编排层巨型函数是最大债务 |
| 测试覆盖 | C | 几何解析扎实（222 用例全通过），编排/线路/坐标对齐空白 |
| 后端质量 | B+ | 无注入/无 panic/事务正确，但单文件 3544 行 |
...
[viewerRuntime.ts:4](file:///d:/vibe-coding/gim_viewer/src/viewer/viewerRuntime.ts#L4) 静态导入 `container` from `ui/dom.js`，[:34-35](file:///d:/vibe-coding/gim_viewer/src/viewer/viewerRuntime.ts#L34-L35) 动态导入 `ui/propsDrawer`、`ui/modelList`。3D 渲染层反向耦合到特定 DOM 结构，根因是 viewport 元素未通过参数注入。

**影响**：viewer 层无法独立测试或复用，任何 DOM 结构变更会波及 3D 层。

### 2.2 ui 层直接调用 gim 解析器

| 文件 | 行 | 导入的 gim 解析器 |
|---|---|---|
| [propsDrawer.ts](file:///d:/vibe-coding/gim_viewer/src/ui/propsDrawer.ts#L5-L7) | 5-7 | `parseFamSections`、`parseKeyValue`、`getNodeDisplayName` |
| [cbmTreeView.ts](file:///d:/vibe-coding/gim_viewer/src/ui/cbmTreeView.ts#L4) | 4 | `getNodeDisplayName` |
| [fileDevView.ts](file:///d:/vibe-coding/gim_viewer/src/ui/fileDevView.ts#L4) | 4 | `getNodeDisplayName` |

AGENTS.md 声明 ui"不直接碰数据库和 IFC Loader"，但 ui 绕过编排层直接调用 gim 纯解析器。

**影响**：ui 层与 gim 解析器耦合，解析逻辑变更需同步修改 ui 层而非仅修改 services 层。

### 2.3 ui→services 运行时依赖

[cacheManagerView.ts:20](file:///d:/vibe-coding/gim_viewer/src/ui/cacheManagerView.ts#L20) 静态导入 `summarizeDiagnostic` from `services/diagnosticSummaryService.js`，与 AGENTS.md 声明的 ui"不直接碰数据库"边界不一致。

### 2.4 AppState 缺乏集中 mutator

[app/state.ts:91-122](file:///d:/vibe-coding/gim_viewer/src/app/state.ts#L91-L122) 的 `resetGimState()` 直接 mutate 20+ 字段，与 [projectCleanupService.ts:188-201](file:///d:/vibe-coding/gim_viewer/src/services/projectCleanupService.ts#L188-L201) 职责重叠：cleanup 在 `resetGimState()` 之后又手动清 `loadedModels`、`highlightedItems`、`hasFittedCamera`（注释说明"resetGimState 不清这两项"）。

**影响**：新增状态字段时需同时修改两处，容易漏清导致 stale 引用。没有集中的 mutator API，状态结构变更波及所有持有 state 的模块。

### 2.5 openGimService.ts 巨型函数

[openGimService.ts](file:///d:/vibe-coding/gim_viewer/src/services/openGimService.ts) 共 **917 行**，其中 [`openGimWithDialog`](file:///d:/vibe-coding/gim_viewer/src/services/openGimService.ts#L747-L917) 函数 **171 行**，包含 **35 处动态 import**，嵌套了缓存命中短路（变电/线路两条恢复路径）和完整解压回退两条主路径。
...
### 3.1 db.rs 单文件 3544 行

[db.rs](file:///d:/vibe-coding/gim_viewer/src-tauri/src/db.rs) 共 **3544 行**。重复严重：
...
[db.rs:2955](file:///d:/vibe-coding/gim_viewer/src-tauri/src/db.rs#L2955) 获取 `Mutex<Connection>` 锁后，[:2961](file:///d:/vibe-coding/gim_viewer/src-tauri/src/db.rs#L2961) 在持锁状态下调用 `query_reachable_geometry`，该函数内部执行 CBM 树遍历 + 子设备递归 + DEV/PHM/MOD 三层 JOIN + 矩阵字符串拼接。锁持有到 :2971 才释放。
...
| 模块 | 行数 | 风险 |
|---|---|---|
| [openGimService.ts](file:///d:/vibe-coding/gim_viewer/src/services/openGimService.ts) | 917 | 主编排器，35 处动态 import，两条恢复路径均未验证 |
| [database.ts](file:///d:/vibe-coding/gim_viewer/src/desktop/database.ts) | 842 | 整个 DB 调用包装层零覆盖 |
...
| [modAutoLoadService.ts](file:///d:/vibe-coding/gim_viewer/src/services/modAutoLoadService.ts) | 982 | MOD/STL 自动加载 |
...
| [nodeInteractionService.ts](file:///d:/vibe-coding/gim_viewer/src/services/nodeInteractionService.ts) | 611 | 节点点击懒加载枢纽 |
...
- **悬链线后置**：`lineMapView.ts:519-538` 线路导线为直线段（moveTo/lineTo），悬链线未实现，符合 M5 后置约束。
...
| P1 | **收敛 `openGimWithDialog`**：缓存命中分支、解压回退分支抽成独立可测函数 | 中 | 减少 35 处动态 import 散布 |
| P2 | **viewer→ui 解耦**：viewport 元素通过参数注入 | 中 | viewer 层可独立测试 |
| P2 | **引入 Rust 结构化错误类型**（thiserror） | 中 | 前端可按类型匹配错误 |
| P2 | **线路工程解析链补测** | 大 | 当前仅变电站路径有测试 |
| P3 | **AppState 集中 mutator API** | 中 | 消除 resetGimState/cleanup 职责重叠 |
