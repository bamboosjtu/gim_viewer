...
把新引入的 `.gim` 样本对照已沉淀的 schema 文档做结构化体检，输出符合 `docs/schema/` 既有风格的分析报告。

**边界**：
- 分析检查对 GIM 样本与数据库只读
- 允许生成 `_generated/` 输出与样本登记文档（`docs/schema/00-sample-corpus.md`）
- 不写 SQLite，不修改 parser，不做渲染实现
...
| [desktop/scripts/gim_survey/device_type_survey.py](../../../desktop/scripts/gim_survey/device_type_survey.py) | Round 8.5 + 8.6 | STL / MOD 设备类型六维度聚合（十样本批量反查） |
...
#### Python 批量验证脚本（2026-08 新增，推荐入口）

十样本全量复核时沉淀的 Python 脚本，位于仓库 `desktop/scripts/gim_survey/`，
跨平台、自动处理 GIMPKGS/GIMPKGT 头部切割与 UTF-8 BOM，适合多样本批量回归：

| 脚本 | 用途 | 对应 Round |
| ---- | --- | ---------- |
| `container_verify.py` | 批量容器层验证（哈希/魔数/头部元数据字段/payload offset） | R1.1 |
| `extract_inventory.py` | py7zr 批量解压 + 文件清单 + 文本/二进制粗判 | R1.2 + R1.3 |
| `ref_chain.py` | 三级引用链提取 + 完整性校验 + ENTITYNAME 分布 + project.cbm 字段 | R2 |
| `mod_reachability.py` | MOD 静态分型（含 .gl）+ primitive 统计 + PHM 递归可达性 + entityName→几何映射 | R3 + R4 |
| `line_grammar.py` | 线路 4 类文本格式族 grammar 批量验证（R/G/P/Bolt/CODE/KV 签名） | R7 |
| `transform_stl.py` | PHM/MOD 矩阵分类 + STL 格式/覆盖/复用统计 | R5 + R8.1/R8.2 |

运行方式：`python desktop/scripts/gim_survey/<script>.py`（无需参数，扫描 demo/ 全部在册样本）。

### 2.4 已登记样本对照集

当前样本库（2026-08 扩样后）：4 变电 + 6 线路，详见 [00-sample-corpus.md](../../../docs/schema/00-sample-corpus.md)：

- 变电：demo-substation（JinQu）、substation02（Bentley）、substation03（BIMBase）、substation04（SDDP）
- 线路：demo-line1、line02、line03、line04、line05、line06（SLW3D ×5 + 输电线路数字化勘测设计系统 ×1）

新样本必须与同类型样本对照（线路 vs 线路，变电 vs 变电），不能跨类型直接对比；
变电样本还需标注导出软件——多项基线（PHM 矩阵、STL 设备类型、primitive 家族、payload offset）已被证实随导出软件变化。

**匿名化约定**：写入 `docs/schema/` 的任何内容不得包含真实工程名、建设单位、设计单位、
内部编号或地理归属；样本一律使用中性编号（变电站01..04 / 线路01..06，ASCII ID 同步中性化）。
原 demo-line（某500kV线路）已退出样本库，其历史结论降级为参考。
...
**采集原则**（与 `05-gim-reference-integrity.md` 一致）：
- **优先按 VALUE 是否带文件扩展名做全量扫描**：`.cbm` / `.dev` / `.phm` / `.fam` / `.ifc` / `.mod` / `.stl`
- 不要只依赖字段名白名单；字段名白名单仅用于归类，不用于决定是否采集引用
- 任何 `KEY=VALUE` 中 VALUE 带上述扩展名的，都视为一条引用并入库

**步骤**：
1. 扫描所有 `.cbm` 文件，按 `KEY=VALUE` 解析
2. 对所有 KEY，检查 VALUE 是否以已知扩展名结尾（大小写不敏感）
3. 按扩展名归类引用类型：
   - `.dev` → DEV 引用（含 `OBJECTMODELPOINTER` 与 `FILE.N.DEVn`）
   - `.fam` → FAM 引用（含 `BASEFAMILY` 与 `BASEFAMILY1..N`）
   - `.cbm` → CBM 引用（见下方字段示例，**不要硬编码只识别 SUBDEVICE**）
   - `.ifc` → IFC 引用（含 `IFCFILE`）
   - `.phm` / `.mod` / `.stl` → 几何资源引用（CBM 不直接引用，但全量扫描时若发现要保留）
4. 注意：部分 CBM 中 `OBJECTMODELPOINTER` 出现在 `ENTITYNAME` 之前，需两遍扫描
5. 导出 `_generated/<sampleId>/<sampleId>-cbm-refs.csv`

**CBM → CBM 引用字段示例**（与 `04-cbm-field-dictionary.md` §7 主层级字段集一致，**仅用于归类参考，不是采集白名单**）：

```text
SUBSYSTEMn        SECTIONn           STRAINSECTIONn
GROUPn             SUBDEVICEn        TOWERn
BASEn              STRINGn.STRING    BACKSTRING
FRONTSTRING        FILE.N.DEVn
```

> **关键修正**：原版本只识别 `SUBDEVICEn`，遗漏了线路工程的主层级字段 `SECTIONn` / `STRAINSECTIONn` / `GROUPn` / `TOWERn` / `BASEn` / `STRINGn.STRING` / `BACKSTRING` / `FRONTSTRING` / `FILE.N.DEVn`。这会导致用 skill 分析新线路样本时再次漏掉关键 CBM 引用。务必按 VALUE 扩展名做全量扫描。
...
**步骤**：
1. 扫描所有 `.phm` 文件
2. 收集 `SOLIDMODELn` → MOD 或 STL（按扩展名归类）
3. **实现上不要硬编码只允许 MOD/STL**：按扩展名分发，未知目标（如 `.phm`、`.ifc`）进入诊断 CSV
4. 导出 `_generated/<sampleId>/<sampleId>-phm-refs.csv`

**当前样本依据**：
- 三样本（demo-line / demo-line1 / demo-substation）PHM 的 SOLIDMODEL 引用目标全部为 `.mod` 或 `.stl`
- 未观察到 PHM → PHM 的引用（详见 `05-gim-reference-integrity.md` 与 `07-dev-phm-geometry-reachability.md`）
- 保留"未知扩展名进入诊断"的兼容性提醒，避免新样本出现 PHM → PHM 时被静默丢弃
...
**目标**：通过 CBM → DEV → PHM → STL 完整链反查 STL 对应的具体设备类型，回答 "STL 主要渲染什么设备"。

**步骤**：
1. Pass 1：扫描所有 STL 文件，记录 format/size/triangles（与 R8.1 一致）
2. Pass 2：扫描所有 PHM 文件，提取 `SOLIDMODELn` 中的 .stl 与 .mod 引用，建立 PHM → STL/MOD 映射
3. Pass 3：扫描所有 DEV 文件，提取 SOLIDMODEL + SUBDEVICE 双源引用，建立 DEV → PHM / DEV → DEV 递归映射 + SYMBOLNAME / TYPE 字段
4. Pass 4：扫描所有 CBM 文件（按 SampleId 过滤 entityName）：
   - 变电样本：F4System/PARTINDEX + SYSCLASSIFYNAME/PARTNAME
   - 线路样本：Tower_Device/Wire_Device/CROSS/WIRE/F4System + NAME → CLASSIFYNAME → DEVICETYPE → TOWERTYPE 兜底链 + FAM 装置名称/型号补齐
5. Pass 5：六维度聚合统计（entityName / SYMBOLNAME / TYPE / CLASSIFYNAME / stlSize / ifcFile），输出 Top 5 表格

**判定**：
- 变电样本（demo-substation）：STL 100% 渲染二次设备柜（TYPE=SecondaryCabinet），Top 5 为光纤配线柜/220kV线路保护测控柜/主变电能表柜/综合配线柜/光传输设备柜1
- 线路样本（demo-line / demo-line1）：STL 100% 渲染金具（SYMBOLNAME=INSULATOR/SPACER/DAMPER），不渲染塔本体
- 三样本对比：线路 STL 渲染"线路金具"（绝缘子/间隔棒/防振锤），变电 STL 渲染"二次设备柜"（通信柜/保护测控柜），两类工程的 STL 设备类型完全不同
- 若新样本出现新的 SYMBOLNAME/TYPE 分布，需在报告中记录

**脚本**：[desktop/scripts/gim_survey/device_type_survey.py](../../../desktop/scripts/gim_survey/device_type_survey.py)
**输出**：`_generated/stl-device-type-survey-<sampleId>.json`
**对照基线**：`docs/schema/12-stl-static-survey.md` §9

#### R8.6 MOD 设备类型对比调研

**目标**：通过 CBM → DEV → PHM → MOD 完整链反查 MOD 对应的具体设备类型，回答 "MOD 主要渲染什么设备"，并与 STL 设备类型对比。

**步骤**：
1. Pass 1：扫描所有 MOD 文件，通过 `Classify-ModText` 分类（XML_WITH_ENTITIES / TEXT_HNUM_COMMA_RECORD / TEXT_POINT_LINE / TEXT_SECTION_KV_RECORD / TEXT_KEY_VALUE 等）
   - 变电样本：XML_WITH_ENTITIES 进一步解析为 14 种 primitive 类型
   - 线路样本：4 类文本格式族（不解析 primitive，仅记录 kind）
2. Pass 2-4：复用 R8.5 的 PHM/DEV/CBM 扫描与递归追踪逻辑
3. Pass 5：六维度聚合（entityName / SYMBOLNAME / TYPE / CLASSIFYNAME / modKind / primitiveType），输出 Top 5 表格
4. 额外聚合 modKind → 设备类型对应关系（用于线路样本的 4 类文本格式族与设备归属）

**判定**：
- 变电样本（demo-substation）：
  - MOD 渲染一次电气设备（电容器/GIS/接地变/避雷器等，4179 CBM，14 种 primitive）
  - Top 5 TYPE：OTHERS + SecondaryCabinet + HGIS + FrameCapacitor + GroundTransformer
  - STL vs MOD 分工清晰：STL=二次柜三角网格，MOD=一次设备参数化 primitive
- 线路样本（demo-line / demo-line1）：
  - MOD 100% 渲染"塔 + 线 + 跨越点"（Tower_Device 杆塔主体 + WIRE 导线参数 + CROSS 跨越点）
  - 4 类文本格式族与设备类型 100% 单射对应：TEXT_HNUM→TOWER/Tower_Device、TEXT_POINT_LINE→EQUIPMENT/CROSS、TEXT_SECTION_KV→BASE/Tower_Device（螺栓表）、TEXT_KEY_VALUE→WIRE+BASE（按 key 大小写二分）
  - 不渲染金具（金具由 STL 承载，Wire_Device 100% STL-only）
- 三样本对比：线路 MOD 渲染"杆塔 + 导线 + 跨越点"（输电线路本体），变电 MOD 渲染"一次电气设备"，两类工程 MOD 设备类型与格式体系完全不同
- 若新样本偏离上述模式，需在报告中详细记录

**脚本**：[desktop/scripts/gim_survey/device_type_survey.py](../../../desktop/scripts/gim_survey/device_type_survey.py)
**输出**：`_generated/mod-device-type-survey-<sampleId>.json`
**对照基线**：`docs/schema/12-stl-static-survey.md` §10
...
**步骤**：
1. 7 阶段实施：
   - 阶段 1：IR schema 落地（desktop/src/gim/geometry/ir.ts）
   - 阶段 2：xml-mod 渲染（P0）
...
| 7 | 引用完整性硬缺失率 | 趋近于 0（10/10 样本实测为 0） | 显著大于 0 需列明细 |
| 8 | PHM 嵌套深度 | 旧三样本=1；BIMBase 样本实测最深 4；遍历必须递归+防环 | 深度 >4 或出现环需标记异常（2026-08 更新） |
| 9 | DEV 是否有环 | 无 | 有环需标记异常 |
| 10 | CBM→DEV 入口对齐 | 根 DEV 都被 CBM 引用 | 孤儿 DEV 需列明细 |
| 11 | MOD 静态分类 | 线路 4 类文本格式族、变电 2 类 XML 格式族（10/10 零逃逸） | 新格式族需更新分类函数 |
| 12 | CBM→MOD 可达率 | 接近但不必等于 100%；孤儿规模跨样本差异大（0~475），且可为带 Entity 的 XML MOD | 显著偏低需做孤儿溯源（2026-08 更新） |
| 13 | IFCGUID 命中（变电） | 强命中率应高 | 硬缺失需列明细 |
| 14 | PHM 矩阵分类 | 不再恒为 IDENTITY：JinQu/Bentley/线路全 IDENTITY；BIMBase/SDDP 含大量平移+旋转矩阵 | 非 IDENTITY 属正常形态，渲染必须应用 PHM 矩阵（2026-08 推翻旧基线） |
| 15 | PHM × MOD 两级变换 | 随导出软件而异：部分工具两级变换成立，部分单级 | 渲染管线无条件应用两级矩阵（2026-08 更新） |
...
| 21 | TEXT_HNUM_COMMA_RECORD R token 变体 | 11 主导、5 常见、9 极罕见（仅 demo-line1 2 条） | 新变体需记录并加弱 schema（2026-08 更新占比） |
| 22 | TEXT_POINT_LINE POINT/LINE token 数 | POINT=5, LINE=2（恒定） | ≠ 5 或 ≠ 2 需标记异常 |
| 23 | TEXT_SECTION_KV_RECORD section header | header 恒为 `Bolt`；BoltNum 动态读取（已知 4/8/12）；BoltN 值首段恒 12 token、分号段数 {2,6} 两形态 | 禁止写死 BoltNum 与段数（2026-08 修正：首段 12 而非 15） |
| 24 | TEXT_KEY_VALUE 签名 | 至少 5 种：Tower 型含 D / 含 α1α2 / 无 e1e2 / 超长 ZPOSTARRAY 变体 + WIRE 大写型 | 新签名需加弱 schema 兜底（2026-08 扩展） |
| 25 | STL 格式 | 以 binary 为主；ASCII STL 已实证存在（`solid AssimpScene` header） | loader 必须兼容 ASCII 分支（2026-08 修正） |
| 26 | STL PHM 引用率与复用 | 覆盖率 100% 且零复用（每 STL 恰被 1 个 PHM 引用 1 次） | < 100% 或出现复用需记录（2026-08 补充） |
...
| 31 | IR 两级变换表述 | 结构保留两级字段；PHM 矩阵是否 IDENTITY 因导出软件而异，渲染必须应用 | 把"PHM 恒 IDENTITY"写死进实现需修正（2026-08 更新） |
...
| 35 | 变电 STL 设备类型 | 因导出软件而异：JinQu=SecondaryCabinet 二次柜；BIMBase=一次设备（避雷器/接地开关/电容器组）；Bentley 可能无 STL | 不能假设变电 STL 设备归属（2026-08 推翻单一归属） |
| 36 | 线路 STL 设备类型 | 金具（INSULATOR/FITTINGS/SPACER/DAMPER），不渲染塔本体（6/6 样本复现） | 出现塔本体 STL 需详细分析 |
| 37 | 变电 MOD 设备类型 | 一次电气设备（电容器/GIS/接地变/避雷器等，14 种 primitive） | 出现二次柜主导 primitive 需记录 |
| 38 | 线路 MOD 设备类型 | entityName→kind 映射 6/6 样本复现：CROSS→POINT_LINE(100%)、WIRE→KEY_VALUE(100%)、WIRE_DEVICE→STL(100%)、TOWER_DEVICE→HNUM+SECTION_KV+KEY_VALUE(+STL) | 出现跨 kind 混用需详细分析（2026-08 更新） |
