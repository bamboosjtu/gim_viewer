...
export function parseHNumCommaRecord(text: string, modPath: string): HNumModFile {
  ...
  for (const rawLine of lines) {
    ...
    if (m) {
      hNum = parseBoundedCount(m[1], 'HNum');
      continue;
    }
    // H,27000,Body1,Leg1
    ...
    if (m) {
      if (currentBody && currentBody.groundPoints.length >= PARSER_LIMITS.maxGeometryInstances) throw new Error(`G 记录数超过安全上限 ${PARSER_LIMITS.maxGeometryInstances}`);
      const g: GRecord = {
        type: m[1],
        name: m[2],
        x: parseFloat(m[3]),
    ...
    if (m) {
      if (hSubLegs.length >= PARSER_LIMITS.maxGeometryInstances) throw new Error(`HSubLeg 记录数超过安全上限 ${PARSER_LIMITS.maxGeometryInstances}`);
      hSubLegs.push({
    ...
    m = RE_HLEG.exec(line);
    if (m) {
      if (hLegs.length >= PARSER_LIMITS.maxGeometryInstances) throw new Error(`HLeg 记录数超过安全上限 ${PARSER_LIMITS.maxGeometryInstances}`);
      hLegs.push({
        index: parseInt(m[1], 10),
        x: parseFloat(m[2]),
        y: parseFloat(m[3]),
...
export function parsePointLine(text: string, _modPath: string): PointLineModFile {
  ...
  for (const rawLine of lines) {
    ...
    if (m) {
      pointNum = parseBoundedCount(m[1], 'POINTNUM');
      continue;
    }
    m = RE_LINENUM.exec(line);
    if (m) {
      lineNum = parseBoundedCount(m[1], 'LINENUM');
      continue;
    }
    m = RE_POINT.exec(line);
    if (m) {
      if (points.length >= PARSER_LIMITS.maxGeometryInstances) throw new Error(`POINT 记录数超过安全上限 ${PARSER_LIMITS.maxGeometryInstances}`);
      points.push({
        id: parseInt(m[2], 10),
        lat: parseFloat(m[3]),
        lon: parseFloat(m[4]),
    ...
    m = RE_LINE.exec(line);
    if (m) {
      if (lines_.length >= PARSER_LIMITS.maxGeometryInstances) throw new Error(`LINE 记录数超过安全上限 ${PARSER_LIMITS.maxGeometryInstances}`);
      lines_.push({
...
export function parseSectionKvRecord(text: string, modPath: string): BoltModFile {
  ...
  for (const rawLine of lines) {
    ...
    if (m) {
      boltNum = parseBoundedCount(m[1], 'BoltNum');
      continue;
    }

    m = RE_BOLT_N.exec(line);
    if (m) {
      if (bolts.length >= PARSER_LIMITS.maxGeometryInstances) throw new Error(`Bolt 记录数超过安全上限 ${PARSER_LIMITS.maxGeometryInstances}`);
