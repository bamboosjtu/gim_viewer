...
export type XmlModPrimitive =
  ...
  /** 变电软母线（Bentley/BIMBase/SDDP）：FitCoordArray 拟合点序列，绝对 mm 坐标展平为 [x,y,z,...] */
  | { type: "Wire"; d: number; fitPoints: number[] }
  /** 电缆敷设路径：InflectionCoordArray + Start/End，绝对 mm 坐标展平 */
  | { type: "Cable"; d: number; inflectionPoints: number[] }
  /** 旋转椭球 */
  | { type: "RotationalEllipsoid"; h: number; lr: number; wr: number }
  /** 槽钢类钢（BeamChannel/LightBeamChannel/H）：截面由型号查表，暂用近似盒体 */
  | { type: "BeamChannelLike"; length: number; model: string }
  /** 布尔运算（Bentley/BIMBase）：引用同 MOD 文件内的两个 Entity ID，可链式嵌套 */
  | {
      type: "Boolean";
 ...
 *
 * A 原始值 0-255：刻度随导出工具而异（百分制 vs 字节制），
 * 判定规则与不透明哨兵见 docs/schema/10-substation-mod-grammar.md §P3-2。
 */
export interface XmlModColor {
  r: number; // 0-255
  g: number; // 0-255
  b: number; // 0-255
  a: number; // 0-255（原始值；刻度由文件级 colorMaxA 判定）
}
...
export interface PhmDocument {
  ...
  isEmpty: boolean;
  /** 文件内 COLORn.A 的最大原始值，用于判定百分制/字节制透明度刻度。 */
  colorMaxA?: number;
}
