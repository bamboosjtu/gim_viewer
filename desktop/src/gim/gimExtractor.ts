...
export const EXTRACTION_LIMITS = Object.freeze({
  ...
  /** 解压展开总量 / 压缩 payload 大小上限，防止高压缩比炸弹。 */
...
function flattenExtractedFiles(
  ...
  prefix = '',
  depth = 0,
  stats: { entries: number; totalBytes: number } = { entries: 0, totalBytes: 0 },
): Map<string, File> {
  const result = new Map<string, File>();
  if (!obj || typeof obj !== 'object') return result;
  if (depth > EXTRACTION_LIMITS.maxNestingDepth) {
    throw new Error(`解压目录层级超过 ${EXTRACTION_LIMITS.maxNestingDepth} 层`);
  }
  for (const [key, value] of Object.entries(obj as Record<string, unknown>)) {
    const path = prefix ? `${prefix}/${key}` : key;
    const normalized = path.replace(/\\/g, '/');
    if (UTF8_ENCODER.encode(normalized).byteLength > EXTRACTION_LIMITS.maxPathBytes) {
      throw new Error(`解压条目路径过长（>${EXTRACTION_LIMITS.maxPathBytes} 字节）`);
    }
    if (normalized.startsWith('/') || /^[A-Za-z]:\//.test(normalized)
      || normalized.split('/').some((part) => part === '..' || part === '.' || part === '')) {
      throw new Error(`解压条目路径不安全: ${path}`);
    }
    if (value instanceof File) {
      if (stats.entries >= EXTRACTION_LIMITS.maxEntries) {
        throw new Error(`解压条目数超过 ${EXTRACTION_LIMITS.maxEntries}`);
      }
      if (value.size > EXTRACTION_LIMITS.maxFileBytes) {
        throw new Error(`解压单文件超过 ${EXTRACTION_LIMITS.maxFileBytes} 字节: ${normalized}`);
      }
      const nextTotal = stats.totalBytes + value.size;
      if (!Number.isSafeInteger(nextTotal) || nextTotal > EXTRACTION_LIMITS.maxTotalUncompressedBytes) {
        throw new Error(`解压总大小超过 ${EXTRACTION_LIMITS.maxTotalUncompressedBytes} 字节`);
      }
      stats.entries++;
      stats.totalBytes = nextTotal;
      result.set(normalized, value);
    } else if (value && typeof value === 'object') {
      for (const [sp, sf] of flattenExtractedFiles(value, normalized, depth + 1, stats)) result.set(sp, sf);
    }
  }
  return result;
}
...
export async function extractGimFile(arrayBuffer: ArrayBuffer): Promise<Map<string, File>> {
  if (arrayBuffer.byteLength > EXTRACTION_LIMITS.maxArchiveBytes) {
    throw new Error(`GIM 文件大小超过 ${EXTRACTION_LIMITS.maxArchiveBytes} 字节`);
  }
  ...
  try {
    ...
    const files = flattenExtractedFiles(extracted);
    const compressedBytes = Math.max(1, ab.byteLength);
    let totalBytes = 0;
    for (const file of files.values()) {
      totalBytes += file.size;
      if (!Number.isSafeInteger(totalBytes) || totalBytes > EXTRACTION_LIMITS.maxTotalUncompressedBytes) {
        throw new Error(`解压总大小超过 ${EXTRACTION_LIMITS.maxTotalUncompressedBytes} 字节`);
      }
      if (totalBytes > compressedBytes * EXTRACTION_LIMITS.maxCompressionRatio) {
        throw new Error(`解压压缩比超过 ${EXTRACTION_LIMITS.maxCompressionRatio}×，疑似压缩炸弹`);
      }
    }
    return files;
  } finally {
