...
import { findFileByPath } from './fileLookup.js';
...
export async function buildLineGimGraph(
  ...
  for (const candidate of ['Cbm/project.cbm', 'CBM/project.cbm', 'cbm/project.cbm']) {
    const entry = findFileByPath(files, candidate);
    if (entry) { entryPath = entry.path; break; }
  }
  if (!entryPath) {
    // 回退：取第一个 .cbm 文件作为入口
  ...
  async function resolveCrossCode(
    ompValue: string,
    fileByName: Map<string, { path: string; file: File }>,
  ): Promise<string | null> {
    try {
      const readKv = async (name: string): Promise<Record<string, string> | null> => {
        const entry = fileByName.get(name.toLowerCase());
        if (!entry) return null;
        return parseKeyValue(await entry.file.text());
      };
      const devName = extractFileName(ompValue);
      if (!devName.endsWith('.dev')) return null;
      const dev = await readKv(devName);
      const phmRaw = dev?.['SOLIDMODEL0'];
      if (!phmRaw || !phmRaw.toLowerCase().endsWith('.phm')) return null;
      const phm = await readKv(extractFileName(phmRaw));
      const modRaw = phm?.['SOLIDMODEL0'];
      if (!modRaw || !modRaw.toLowerCase().endsWith('.mod')) return null;
      const modEntry = fileByName.get(extractFileName(modRaw).toLowerCase());
      if (!modEntry) return null;
      for (const line of (await modEntry.file.text()).split(/\r?\n/)) {
  ...
  async function build(nodePath: string, depth = 0): Promise<GimGraphNode | null> {
    ...
    if (subSystemVal) {
      ...
      const child = await build(subSystemVal, depth + 1);
    ...
    for (const spec of ARRAY_REF_SPECS) {
      ...
      for (let i = 0; i < num; i++) {
        ...
        if (v) {
          ...
          if (spec.recursive) {
            const child = await build(v, depth + 1);
            if (child) children.push(child);
          }
        }
      }
    ...
    for (let i = 0; i < stringsNum; i++) {
      ...
      if (strVal) {
        ...
        const child = await build(strVal, depth + 1);
        if (child) children.push(child);
      }
      if (gpoint) {
