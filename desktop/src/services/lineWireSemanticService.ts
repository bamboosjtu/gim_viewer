...
interface WireLike {
  ...
  endLng: number;
  wireType: string;
  /** 起点高程（米，BLHA 第三段），缺失为 null */
  startElev?: number | null;
  /** 终点高程（米，BLHA 第三段），缺失为 null */
  endElev?: number | null;
  /** 节点引用（含 rawProps），可选 */
...
export function buildWireSemanticInfo(args: {
  ...
  if (
    ...
    const planar = haversineMeters(
      wire.startLat, wire.startLng,
      wire.endLat, wire.endLng,
    );
    // 高程差修正：BLHA 含高程时按斜距 sqrt(d²+h²)，否则退化平面距离
    if (
      Number.isFinite(wire.startElev ?? NaN) &&
      Number.isFinite(wire.endElev ?? NaN)
    ) {
      const dElev = (wire.endElev as number) - (wire.startElev as number);
      spanMeters = Math.sqrt(planar * planar + dElev * dElev);
    } else {
      spanMeters = planar;
    }
