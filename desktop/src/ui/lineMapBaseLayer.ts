 ...
 *
 * 坐标系决策（dev-log「坐标偏移」项，2026-08 已核验）：
 * BLHA 为勘测源数据（CGCS2000/WGS-84，二者差异厘米级），OSM raster 也是
 * WGS-84，直接叠加即为正确对齐；GCJ-02 是国内 Web 地图服务商在展示层
 * 加的偏移混淆，不存在于工程勘测数据中，若做 WGS-84→GCJ-02 转换反而
 * 会引入 300~600m 的错误位移。故本层不做任何坐标偏移转换。
 */
