...
export function createTiandituSatelliteStyle(): StyleSpecification {
  ...
  return {
    ...
    sources: {
      'tianditu-img': {
        type: 'raster',
        tiles: tiandituTiles('img_w'),
        tileSize: 256,
        attribution: TIANDITU_ATTRIBUTION,
      },
      'tianditu-cia': {
        type: 'raster',
        tiles: tiandituTiles('cia_w'),
        tileSize: 256,
      },
    },
    layers: [
...
export function createTiandituTerrainStyle(): StyleSpecification {
  ...
  return {
    ...
    sources: {
      'tianditu-ter': {
        type: 'raster',
        tiles: tiandituTiles('ter_w'),
        tileSize: 256,
        attribution: TIANDITU_ATTRIBUTION,
      },
      'tianditu-cta': {
        type: 'raster',
        tiles: tiandituTiles('cta_w'),
        tileSize: 256,
      },
    },
    layers: [
...
export function createTiandituVectorStyle(): StyleSpecification {
  ...
  return {
    ...
    sources: {
      'tianditu-vec': {
        type: 'raster',
        tiles: tiandituTiles('vec_w'),
        tileSize: 256,
        attribution: TIANDITU_ATTRIBUTION,
      },
      'tianditu-cva': {
        type: 'raster',
        tiles: tiandituTiles('cva_w'),
        tileSize: 256,
      },
    },
    layers: [
...
/** 天地图 tiles 数组：全部子域参与负载均衡，提升瓦片并行加载速度 */
function tiandituTiles(layer: TiandituLayerType): string[] {
  return TIANDITU_SUBDOMAINS.map((sub) => buildTiandituTileUrl(layer, sub));
}
