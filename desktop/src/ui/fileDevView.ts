...
export function renderFileDevPanelUI(
  ...
  // FileDevRelation 的设备引用在真实样本中同时存在 `foo.cbm`、`CBM/foo.cbm`、
  // 反斜杠和大小写变体。构建一次路径/文件名索引，避免展开面板时因直接
  // state.cbmNodeIndex.get(rawValue) 失败而显示成不可点击的“幽灵设备”。
  const cbmReferenceIndex = buildCbmReferenceIndex(state.currentCbmTree);

  const modDeviceNodes = collectModDeviceNodes(state.currentCbmTree);
  const hasDesignEntries = state.fileDevRelations.length > 0;
  const hasModDevices = modDeviceNodes.length > 0;

  if (!hasDesignEntries && !hasModDevices) {
  ...
  for (const entry of state.fileDevRelations) {
    const isIfcSource = /\.ifc$/i.test(entry.ifcFile);
    const { nodeEl, row, childrenEl } = createCollapsibleNode(
      isIfcSource ? '📄' : '🗺️',
      `${entry.ifcName || entry.ifcFile || '未命名来源'} (${entry.deviceCount})${isIfcSource ? '' : ' · 来源图纸'}`,
      '0px',
    );
    ...
    row.addEventListener('click', () => {
      ...
      if (expanded && !childrenRendered) {
        for (const devCbm of entry.deviceCbms) {
          const devNode = resolveCbmReference(cbmReferenceIndex, devCbm);
...
function buildCbmReferenceIndex(root: CbmNode | null): Map<string, CbmNode> {
  const index = new Map<string, CbmNode>();
  if (!root) return index;
  const walk = (node: CbmNode): void => {
    for (const key of cbmReferenceKeys(node.path)) {
      // 完整路径优先；同名 basename 冲突时保留第一个确定结果，
      // 但完整路径仍能命中正确节点。
      if (!index.has(key) || key.includes('/')) index.set(key, node);
    }
...
function resolveCbmReference(index: Map<string, CbmNode>, reference: string): CbmNode | undefined {
  for (const key of cbmReferenceKeys(reference)) {
