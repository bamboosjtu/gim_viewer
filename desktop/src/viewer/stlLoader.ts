...
export function parseStlBinary(buffer: ArrayBuffer, stlPath: string): THREE.Group | null {
  ...
  if (looksSolid) {
    const declaredCount = new DataView(buffer).getUint32(80, true);
    const binaryPlausible = buffer.byteLength >= 84 && 84 + declaredCount * 50 === buffer.byteLength;
    if (!binaryPlausible) {
      return parseStlAscii(buffer, stlPath);
    }
    // 长度公式命中 → 实为二进制，继续走二进制分支
  }
...
/**
 * 解析 ASCII STL 文件（"solid ... facet normal ... endsolid"）。
 *
 * line03（非 SLW3D 工具产出）实测存在 3 个 ASCII STL（header `solid AssimpScene`）。
 * 顶点同样烘焙 mm→m 缩放；法向量优先取 facet normal。
 */
export function parseStlAscii(buffer: ArrayBuffer, stlPath: string): THREE.Group | null {
  const text = new TextDecoder().decode(buffer);
  if (!text.startsWith('solid')) {
    console.warn(`[stlLoader] ASCII STL 不以 solid 开头，跳过: ${stlPath}`);
    return null;
  }

  const positions: number[] = [];
  const normals: number[] = [];
  let curNormal: [number, number, number] | null = null;
  let triCount = 0;
  const MAX_TRIANGLES = 5_000_000;
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    if (line.startsWith('facet normal')) {
      ...
      curNormal = [parseFloat(p[2]), parseFloat(p[3]), parseFloat(p[4])];
    } else if (line.startsWith('vertex')) {
      const p = line.split(/\s+/);
      positions.push(parseFloat(p[1]), parseFloat(p[2]), parseFloat(p[3]));
      const n = curNormal ?? [0, 0, 0];
      normals.push(n[0], n[1], n[2]);
      if (positions.length % 9 === 0) triCount++;
      if (triCount > MAX_TRIANGLES) {
        console.warn(`[stlLoader] ASCII STL 三角形数量过多（>${MAX_TRIANGLES}），跳过: ${stlPath}`);
        return null;
      }
    }
  }
  ...
  if (triCount === 0) {
    console.warn(`[stlLoader] ASCII STL 无有效三角面，跳过: ${stlPath}`);
    return null;
  }
  ...
  // 烘焙 mm→m 缩放到顶点（与二进制分支一致）
  geometry.scale(STL_MM_TO_SCENE_UNIT, STL_MM_TO_SCENE_UNIT, STL_MM_TO_SCENE_UNIT);

  const mesh = new THREE.Mesh(geometry, createDefaultStlMaterial());
  const group = new THREE.Group();
  group.name = stlPath;
  group.add(mesh);
  return group;
...
function createDefaultStlMaterial(): THREE.MeshStandardMaterial {
