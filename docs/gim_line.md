...
| SQLite 缓存（6 张表） | ✅ 已实现 | `src-tauri/src/db.rs`（v5） |
| 悬链线参数**审计**（只读，Ctrl+Shift+C） | ✅ 已实现 | `src/services/lineGeometryAuditService.ts` 等 4 个服务 |
| MOD 文件解析（4 类文本格式族） | ❌ 未实现 | 仅有引用记录，无 parser |
| 3D 杆塔渲染（TEXT_HNUM_COMMA_RECORD） | ❌ 未实现 | — |
| Bolt 属性面板（TEXT_SECTION_KV_RECORD） | ❌ 未实现 | — |
| Tower_Device/WIRE 参数面板（TEXT_KEY_VALUE） | ❌ 未实现 | — |
| STL 渲染（Wire_Device 100% 触达 STL） | ❌ 未实现 | — |
| 悬链线/弧垂渲染 | ❌ 未实现 | 仅审计字段，未渲染 |
| 3D 线路（独立 viewer） | ❌ 未实现 | — |
...
| 悬链线暂缓 | M4 已审计但未实现，详见 §11 与 [dev-log.md](dev-log.md) 暂缓项 | §12 P2 |

---

## 11. WIRE 拓扑分类与悬链线候选字段

> 本节归纳 M4-B3 / B3A / B3B / B3C 审计的**已证实**结论。
>
> 待确认的语义假设与暂缓项见 [dev-log.md](dev-log.md) "M4 悬链线暂缓项"。
>
> 研究方法论、审计流程与决策路径见 [m4-research.md](m4-research.md)。
...
| `KVALUE` | 数值类型 | ⏳ 待确认（疑似张力/弧垂参数，见 dev-log） |
| `SPLIT` | 取值 `1` / `4`，正整数 | ⏳ 候选（疑似分裂数，已用于样式加粗） |
| `POINT0.MATRIX0` | 16 元素 4x4 矩阵（逗号分隔） | ⏳ 待确认（疑似挂点平移矩阵） |
| `POINT1.MATRIX0` | 同上 | ⏳ 待确认 |
...
### 11.4 MATRIX0 格式（M4-B3 已证实）

- **格式确认**：16 元素，逗号分隔，为 4x4 矩阵
- **平移分量位置确认**：`values[12]`(x) / `values[13]`(y) / `values[14]`(z)
- **语义未确认**：单位（米/毫米）、坐标系（局部/世界/相对塔位）均待核验 → 见 dev-log 暂缓项
...
- **后续若需要真实导线几何，另起 M5 或专项任务**（详见 [m4-research.md](m4-research.md) §7 决策与后续路线）
...
| **悬链线/弧垂渲染** | inter-point 档距 | 抛物线或悬链线曲线 | 基于 [m4-research.md](m4-research.md) §7 决策；M5 或专项任务 |
