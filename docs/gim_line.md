...
| 悬链线暂缓 | M4 已审计但未实现，详见 §11、[14-line-catenary-study.md](schema/14-line-catenary-study.md) 与 [dev-log.md](dev-log.md) 暂缓项 | §12 P2 |
...
| **缓存命中回放** | geometry_source 表（建议） | 缓存命中时直接恢复 IR | 正式 DDL 另起 [15-geometry-cache-schema.md](schema/15-geometry-cache-schema.md)（待建） |
| **节点联动** | CBM 树 → 3D 高亮 | 选中设备节点 → 高亮对应几何 + 相机定位 | 与变电 IFC 联动模式一致 |
| **悬链线/弧垂渲染** | inter-point 档距 | 抛物线或悬链线曲线 | 基于 [14-line-catenary-study.md](schema/14-line-catenary-study.md) §6 决策；M5 或专项任务 |
