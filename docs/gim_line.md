...
| MOD 文件解析（4 类文本格式族） | ⚠️ Parser 已实现，运行时未接入 | `src/gim/geometry/lineModParser.ts`；grammar 见 [11-line-mod-grammar.md](schema/11-line-mod-grammar.md) |
...
`save_line_project_cache` 是统一事务命令：线路图（6 张表）+ FAM/DEV 属性在同一事务内写入，成功后设置 `parser_version = PARSER_VERSION`（当前为 `gim-parser-v14`，详见 `src-tauri/src/db.rs`；版本变更使旧缓存自动失效以触发完整重建）。
...
| 线路 MOD 运行时未接入 | 四类 parser 已实现，但杆塔/跨越点/参数结果尚未被运行时消费 | §12 P0 |
...
| **line-text-mod parser** | 4 类文本格式族（HNum/PointLine/SectionKV/KeyValue） | ✅ `src/gim/geometry/lineModParser.ts` 已实现；待运行时接入 | 按 [11-line-mod-grammar.md](schema/11-line-mod-grammar.md) §6 demo-line1 已验证；R 记录 9 token 变体保留弱 schema fallback |
