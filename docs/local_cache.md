...
- `SUBSTATION_PARSER_VERSION = gim-substation-parser-v23`：变电 CBM/FAM/DEV、IFC Spatial Semantic 和关系索引；Phase 5 的跨厂商字段/空值语义变化会使旧变电缓存按 miss 重建。
- `PARSER_VERSION = gim-parser-v22` 保留为旧库兼容/诊断字段，不再单独决定线路或变电缓存是否有效。

写入时机：首次导入或重建索引时，线路事务更新 `line_parser_version`，变电事务更新 `substation_parser_version`；旧共享字段同时写入以兼容旧版本诊断。校验时 `validate_gim_cache` 按 `project_type` 选择对应 domain。旧库只有共享字段时，明确识别为线路且版本为 v21/v22 的记录迁移到线路域；Phase 5 以前的变电记录不迁移，按 miss 重建，无法识别工程类型的记录不猜测域并按 miss 处理。
