> **2026-08-24 状态说明**：本报告为早期研究快照（单变电+双线路样本）。
> 权威结论以 `docs/schema/` 编号文档为准，最新基线见 `docs/schema/22-ten-sample-verification-0824.md`
> （10 样本、4 种导出软件；多项单样本结论已被修正或推翻）。
...
- 包头魔数：`GIMPKG`（6 字节前缀），后缀因工程类型而异：
  - `GIMPKGS` — 变电工程
  - `GIMPKGT` — 线路工程
- 7z 签名：`37 7A BC AF 27 1C`
- ZIP 签名：`50 4B 03 04`
- 提取器匹配 `GIMPKG` 前缀（兼容任意 `GIMPKG*` 后缀），并在头部之后的前 1MB 范围内搜索压缩签名以定位真实数据偏移（如 demo-line.gim 的 7z 数据位于偏移 784）
...
### 3.7 线路工程无 IFC 的展示策略（历史可行性评估）

变电工程依赖 web-ifc 解析 IFC 渲染，线路工程**完全无 IFC**。以下表格是早期的几何可行性评估；当前产品已明确以“模型”地图为唯一线路工作区，不启用独立线路 3D viewer。杆塔形状仅在选中杆塔后的属性面板“来源”页提供等比例 HNum 预览，STL/MOD 以来源和参数追溯为主。

| 几何/信息类型 | 数据来源 | 当前产品表达 |
|---|---|---|
| 杆塔钢结构 | .mod (HNum 格式) 的 P/R 记录 | 属性面板“来源”页的 X/Z 等比例 SVG 预览 |
| 绝缘子串/金具 | .stl 二进制 | 来源页可读链接追溯，不创建线路 3D 实例 |
| 导地线悬链线 | WIRE CBM 的 POINT.BLHA + KVALUE + .mod 物理参数 | “模型”地图上的线路表达；参数在属性面板展示 |
| 交叉跨越 | .mod (CODE 格式) 的 POINT/LINE | “模型”地图跨越物标记与属性 |
| 基础螺栓 | .mod (Bolt 格式) | 参数页单表格展示，原始 MOD 在来源页追溯 |

### 3.8 需求与可行性（历史评估，非当前路线图）

| 功能 | 可行性 | 难度 | 依赖 |
|---|---|---|---|
| CBM 层级树展示 | 高 | 中 | 解析多种子节点引用键（SECTION/STRAINSECTION/GROUP/TOWER/STRING/...） |
| 杆塔 3D 渲染 | 可行但不在当前产品路径 | 高 | 自定义 .mod (HNum) 解析器；当前改为来源页 SVG 预览 |
| 导地线悬链线渲染 | 语义待决策 | 高 | BLHA→局部坐标转换 + 悬链线算法；当前只保留地图表达 |
| 绝缘子串渲染 | 可行但不在当前产品路径 | 低 | STLLoader；当前通过来源页追溯 |
| 交叉跨越渲染 | 可行但不在当前产品路径 | 中 | .mod (CODE) 解析；当前以地图标记表达 |
| 属性面板（FAM/DEV/MOD） | 已实现 | 低 | 解析扁平 KEY=VALUE，并按业务对象/属性组展示 |
...
2. **无 IFC 依赖**：线路工程完全脱离 web-ifc；四类 .mod 文本已由解析器接入属性面板，独立线路 3D 不作为当前产品目标
3. **层级引用键多样化**：变电工程统一用 `SUBSYSTEMS.NUM`+`SUBSYSTEM<i>`，线路工程每层不同（SECTION/STRAINSECTION/GROUP/TOWER/STRING/BASE/SUBDEVICE）
4. **地理坐标系**：线路工程全程使用 BLHA（经纬度+方位角），当前投影到“模型”地图；不强制转换到 3D 场景坐标
5. **.mod 格式分裂**：变电工程 .mod 统一为 XML，线路工程 .mod 按用途分 4 种文本格式（HNum/CODE/type/Bolt），已由统一入口按格式分发
