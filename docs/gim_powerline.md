...
`save_line_project_cache` 是统一事务命令：线路图（6 张表）+ FAM/DEV 属性在同一事务内写入，成功后设置 `parser_version = PARSER_VERSION`（当前为 `gim-parser-v19`，详见 `desktop/src-tauri/src/db.rs`；版本变更使旧缓存自动失效以触发完整重建）。
