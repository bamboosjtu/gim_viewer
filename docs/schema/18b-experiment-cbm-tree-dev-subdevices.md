...
[cbmParser.ts](../../desktop/src/gim/cbmParser.ts) 原版名称提取逻辑：
...
DEV 文件有 `SUBDEVICES` 块（递归子 DEV 引用，如屏柜内的开关、继电器）。原版 [cbmParser.ts](../../desktop/src/gim/cbmParser.ts) 只解析 CBM 的 `SUBDEVICES.NUM`+`SUBDEVICEi`（指向子 CBM），**不解析 DEV 文件的 SUBDEVICES 块**。
...
新的优先级链（详见 [cbmParser.ts extractDisplayName](../../desktop/src/gim/cbmParser.ts)）：
...
新增 `expandDevSubDevices` 函数（[cbmParser.ts](../../desktop/src/gim/cbmParser.ts)），在 `buildCbmTree` 的每个节点：
...
[src/gim/types.ts](../../desktop/src/gim/types.ts) `CbmNode` 接口新增 4 个字段：
...
[src/gim/cbmParser.ts](../../desktop/src/gim/cbmParser.ts) 新增：
...
[src/gim/gimIndexer.ts](../../desktop/src/gim/gimIndexer.ts) `getNodeDisplayName` 优先级链：
...
[src/ui/cbmTreeView.ts](../../desktop/src/ui/cbmTreeView.ts)：
...
- [gimIndexRestoreService.ts](../../desktop/src/services/gimIndexRestoreService.ts)：缓存恢复场景（默认空值）
- [fileDevView.ts](../../desktop/src/ui/fileDevView.ts)：临时 CbmNode 构造
- [modGeometryDiscovery.test.ts](../../desktop/src/services/__tests__/modGeometryDiscovery.test.ts)：测试 mock
...
- DEV 文件解析在 [devParser.ts](../../desktop/src/gim/geometry/devParser.ts) 中是纯文本解析，速度很快
- 缓存命中场景（[gimIndexRestoreService.ts](../../desktop/src/services/gimIndexRestoreService.ts)）不触发 DEV 解析（虚拟子节点不入库）
...
**当前实现**：缓存命中时 `buildCbmTree` 不被调用（[gimIndexRestoreService.ts](../../desktop/src/services/gimIndexRestoreService.ts) 从 SQLite 恢复 CbmNode）。
...
- [src/gim/types.ts](../../desktop/src/gim/types.ts)：CbmNode 接口扩展
- [src/gim/cbmParser.ts](../../desktop/src/gim/cbmParser.ts)：核心重写（extractDisplayName + expandDevSubDevices）
- [src/gim/gimIndexer.ts](../../desktop/src/gim/gimIndexer.ts)：getNodeDisplayName 优先级链优化
- [src/ui/cbmTreeView.ts](../../desktop/src/ui/cbmTreeView.ts)：图标映射 + tooltip 增强
- [src/services/gimIndexRestoreService.ts](../../desktop/src/services/gimIndexRestoreService.ts)：兼容性修复
- [src/ui/fileDevView.ts](../../desktop/src/ui/fileDevView.ts)：兼容性修复
- [src/services/__tests__/modGeometryDiscovery.test.ts](../../desktop/src/services/__tests__/modGeometryDiscovery.test.ts)：测试兼容性
...
**文件**：[src/gim/gimExtractor.ts](../../desktop/src/gim/gimExtractor.ts)
...
**文件**：[src/app/state.ts](../../desktop/src/app/state.ts)

新增 `projectName: string` 字段，存储 GIM 头部提取的工程名称，用于 F1System 根节点显示。

#### 11.3.3 openGimService 调用 extractGimHeader

**文件**：[src/services/openGimService.ts](../../desktop/src/services/openGimService.ts)
...
**文件**：[src/gim/cbmParser.ts](../../desktop/src/gim/cbmParser.ts)
...
**文件**：[src/gim/gimIndexer.ts](../../desktop/src/gim/gimIndexer.ts)
...
**文件**：[src/viewer/ifcNameIndex.ts](../../desktop/src/viewer/ifcNameIndex.ts)
...
- **parser_version**：从 `gim-parser-v8` 起逐步升级；本节的 v14 为历史快照，当前源码为 `gim-parser-v22`（演进历史详见 [17-batch-load-schema.md](./17-batch-load-schema.md) §7.3）
...
**文件**：[src/services/modAutoLoadService.ts](../../desktop/src/services/modAutoLoadService.ts)
...
## 15. 第六轮修复（2026-07-11）：DEV_SUBDEVICE 虚拟节点隔离 + 顶点烘焙

### 15.1 问题

方案 B（mergeGeometries 静态合并）实施后，部分 GIS 设备 MOD 位置偏离，跑到了建筑之外。根因有两个：

1. **顶点烘焙缺失**：`applyPlacementTransformToSceneUnits` 使用 `group.applyMatrix4(matrix)`，`Object3D.applyMatrix4` 的实现是 `premultiply + decompose`，当 group 已有 `scale=0.001` 且 placement matrix 含缩放分量 `s` 时，`decompose` 会从 `matrix × Scale(0.001)` 提取 scale，导致 `group.scale` 被错误修改为 `0.001 × s`。变压器 placement 通常纯旋转+平移（`s=1`）所以 OK；GIS 设备 placement 含缩放分量，触发 corrupt。

2. **DEV_SUBDEVICE 虚拟节点 seed 冲突**：18b 把 DEV SUBDEVICES 展开成了 DEV_SUBDEVICE 虚拟 CBM 节点，但几何加载原本已经从真实父 DEV 递归 SUBDEVICES。虚拟节点又被当作全量几何入口后，同一子 DEV 会被两条路径发现，部分路径的矩阵链不完整或被重复覆盖。

### 15.2 修复

#### 15.2.1 顶点烘焙（方案 B 后）

- `collectBakedGeometriesByMaterial`：新增 mm→m 缩放矩阵烘焙到顶点，使 merged geometry 顶点直接以场景单位（米）表达，`group.scale` 保持 1
- `flattenDocumentToGroup`：移除 `merged.scale.setScalar(0.001)`
- `applyPlacementTransformToSceneUnits`：从 `group.applyMatrix4(matrix)` 改为 `group.traverse + mesh.geometry.applyMatrix4(matrix)`（顶点烘焙）
- `stlLoader.ts`：STL 顶点也烘焙 mm→m 缩放（`geometry.scale(0.001, 0.001, 0.001)`）

#### 15.2.2 DEV_SUBDEVICE 虚拟节点隔离

- `modAutoLoadService.ts`：`collectCbmDeviceInstances` 跳过 `entityName === 'DEV_SUBDEVICE'` 的虚拟节点
- `db.rs`：`query_reachable_geometry` 的 SQL 增加 `AND (entity_name IS NULL OR entity_name != 'DEV_SUBDEVICE')` 过滤
- `nodeInteractionService.ts`：点击虚拟节点时调用 `computeCbmParentTransform` 补齐父 CBM 累积矩阵
- `cbmParser.ts`：`expandDevSubDevices` 设置虚拟节点的 `transformMatrix` 为 SUBDEVICE.transformMatrix
- 虚拟节点 path 已带 SUBDEVICE 索引：`${parentCbmPath}#dev:${i}:${childDevPath}`

### 15.3 PARSER_VERSION 升级

`gim-parser-v11` → `gim-parser-v12` → `gim-parser-v13` → `gim-parser-v14`
...
**DEV_SUBDEVICE 虚拟节点的双重角色**：
1. **层级树展示 / 点击定位**：保留虚拟节点用于 UI 展示和点击定位
2. **几何发现**：虚拟节点不作为自动加载 / `get_reachable_geometry` 的 seed；几何发现完全依赖真实 CBM 节点 + DEV SUBDEVICES 递归

**点击虚拟节点时的矩阵补齐**：虚拟节点的 `transformMatrix` 仅含 SUBDEVICE 局部变换，需要补上乘以父 CBM 链累积矩阵后，才能作为 `discoverGeometriesFromNode` 的 root。由 `computeCbmParentTransform` 在 CBM 树中查找目标节点并返回其父链累积矩阵。
