...
> **后续复核**：2026-08-24 已完成十样本全量扩样复核，本轮多项结论已被更新或推翻，
> 最新基线见 [22-ten-sample-verification-0824.md](22-ten-sample-verification-0824.md)。
...
| 11 | A+C | 四类线路 grammar 对两样本成立；parser 已实现且 38 个测试通过，当前由属性面板运行时消费；独立线路 3D 未启用 |
...
| 18b | C | 层级树/DEV SUBDEVICE 演进记录保留；文中 v14 为历史快照，当前 parser 版本为 v22 |
...
1. `demo-line` 的 GIM header 明确为“某500kV线路”；其 FAM 同时出现 AC220kV/AC500kV 设备属性。原文把整个工程归为 220kV、并用电压等级解释 MOD 分布差异，证据不足，已更正。
2. 线路 MOD parser 不是“待实施”：`desktop/src/gim/geometry/lineModParser.ts` 已存在，并由 `lineModRuntimeService` 接入属性面板；当前未完成的是独立线路 3D 几何消费。
...
7. 17/18c 的部分段落仍保留旧版本号；当前源码以 `PARSER_VERSION=gim-parser-v22`、`GEOMETRY_CACHE_VERSION=geometry-cache-v5-dev-status` 为准，旧版本仅作历史记录。
8. 17 号文档把 `46250 / 4135` 同时写成“约 5”和“约 11.2”；已统一为约 11.2 Entity/非空 MOD。

## 6. 仍然有效的风险边界

- 当前只有 2 个线路样本和 1 个变电样本，任何“100%”只代表登记样本，不等同于 GIM 标准强约束。
- 线路 MOD parser 已接入属性面板运行时；grammar 正确不代表线路杆塔、金具和跨越点已经进入独立 3D 视图。
