...
> **2026-08-24 十样本复核**（证据：`_generated/ref-chain-summary.csv` 与 `_generated/<sid>/refs-integrity.csv`，
> 脚本 `desktop/scripts/gim_survey/ref_chain.py`）：
>
> **保持成立的结论**
> - 三层文件级引用完整性在全部 10 个样本均为 **0 缺失**（含 .gl/.ifc 等新目标类型）。
>   「健康 GIM 工程硬缺失率趋近于 0」从三样本事实升级为十样本候选通用规则。
> - 变电 DEV→DEV 组合引用仍全部走 `SUBDEVICEn`（4/4 变电样本），线路走 `SOLIDMODELn=*.dev`（6/6 线路样本）。
>
> **被推翻或需收窄的结论**
> - ~~「未观察到 PHM → PHM 的引用」~~：substation03（BIMBase 产出）存在 **2289 条 PHM→PHM 引用**，
...
>   - **变电站01（JinQu）**：按去重 (IFCFILE, GUID) 对计 3430 对，精确命中 94.81%、硬未命中仅
>     134 个唯一 GUID（3.91%），全部为 F4System + 空 OBJECTMODELPOINTER 节点。
>     旧文档的 74.59% 是按 CBM 节点级引用计数（两个高频空占位 GUID 各被数百个节点重复引用），
>     去重后真实缺失面远小于旧表述——两种口径需并存记录。
>   - **变电站02/变电站03（Bentley/BIMBase）**：设备级 IFCGUID 字段完全不存在（0 个），
>     IFC 仅通过 F1System 级 `IFC.NUM` 清单 + `FileDevRelation.cbm` 关联。
>   - **变电站04（SDDP）**：532 个 IFCGUID 字段全部为空占位，且无 FileDevRelation.cbm。
...
> |---|---|---|---|
> | 总计数键 | `FILE.NUM=24` | **`FILES.NUM=34`** | `FILE.NUM=9` |
> | 子计数键 | `FILEn.DEV.NUM` | **`FILEn.DEVS.NUM`** | `FILEn.DEV.NUM` |
> | IFC 关联 | 奇偶配对：偶=设备列表，奇=`FILEn.IFC=xxx.ifc` | **无 `.IFC` 键**，NAME 直接为图纸文件名（`*.DGN`） | NAME=图纸名 + `FILEn.IFC`（无奇偶配对） |
>
> 结论：fileDevParser 不能写死 JinQu 的奇偶配对结构。需兼容：
...
| 字段模式           | 引用目标        | 说明                         |
| ------------------ | --------------- | ---------------------------- |
| `SOLIDMODELn`      | `.mod` / `.stl` / `.phm` | 实体模型引用，目标为几何图元或嵌套 PHM（`.phm` 仅 substation03/BIMBase 实证） |
