...
> **2026-08-24 十样本复核**：本文为单样本（demo-substation/JinQu）结构事实。新变电样本层级
> ENTITYNAME 存在大小写变体（PartIndex/F4SYSTEM），且变电站02出现 LOGICALMODEL 空壳节点；
> F2/F3/F4 数量比例不能跨工具外推。详见 [22](22-ten-sample-verification-0824.md)。
...
代码层（[cbmParser.ts](../../desktop/src/gim/cbmParser.ts) 第 194-200 行）强制排序为 `[9个U] → [3个A] → [1个S] → [1个G]`，已正确实现。
...
**代码位置**：[gimExtractor.ts](../../desktop/src/gim/gimExtractor.ts) `getProjectTypeName()` → [cbmParser.ts](../../desktop/src/gim/cbmParser.ts) `buildCbmTree(files, projectTypeName)`
...
**代码位置**：[cbmParser.ts](../../desktop/src/gim/cbmParser.ts) `mapF2ClassifyName()` + F1System 子节点排序逻辑
...
**代码位置**：[cbmParser.ts](../../desktop/src/gim/cbmParser.ts) `isDeviceLayer()` + [gimIndexer.ts](../../desktop/src/gim/gimIndexer.ts) `getNodeDisplayName()` + [ifcNameIndex.ts](../../desktop/src/viewer/ifcNameIndex.ts) `isPlaceholderIfcName()`
