...
> **2026-08-24 十样本复核**（证据：`_generated/<sid>/entityname-distribution.csv`，
> 脚本 `desktop/scripts/gim_survey/ref_chain.py`）：
>
> - **ENTITYNAME 大小写三态实证**：键名大小写不敏感的约定升级为强约束——
>   `PARTINDEX`（demo-substation/变电站03/变电站04）/ `PartIndex`（变电站02）；
>   `F4System` / `F4SYSTEM`（变电站04全大写）；线路 `Wire_Device` / `WIRE_DEVICE`（线路03全大写）、
>   `Tower_Device` / `TOWER_DEVICE`。**parser 禁止对 ENTITYNAME 值做精确匹配，必须归一化后比较。**
> - **新实体类型 `LOGICALMODEL`**：仅变电站02（Bentley）出现 1 个，由 project 根级
>   `SUBLOGICALMODELS.NUM=1 + SUBLOGICALMODEL0` 引用；内容仅
>   `ENTITYNAME=LOGICALMODEL` + `LOGICALMODELS.NUM=0`，为无几何的逻辑模型占位节点，
>   parser 可安全识别后跳过。
> - **project.cbm 字段修正**：① `SCH=project.sch`（固定文件名，非 `<uuid>.sch`）；
>   ② BLHA 第 4 值并非恒为 0——变电站02为 `275.98`（方向角候选），变电站04整字段为 `0,0,0,0`；
>   ③ 线路03 header 工程名为“合并”，project.cbm 无 BLHA/TYPE/SCH（与线路一致），
>      但其 ENTITYNAME 用大写 `WIRE_DEVICE/TOWER_DEVICE`——工程类型判定不能依赖 ENTITYNAME 大小写形态。
