...
>
> 本文档只沉淀 schema 与解析管道边界。§1 与 §6 的“现状/当前实现”是本文编写时的历史基线，不代表当前代码现状；当前状态见 [21-schema-conclusion-review-0717.md](21-schema-conclusion-review-0717.md) 和 [gim_powerline.md](../gim_powerline.md)。目前 Geometry IR、变电 XML parser、线路文本 parser、MOD/STL 节点加载和 DEV 粒度 GLB 缓存均已有实现；线路文本 parser 已接入属性检查器，线路产品仍不启用独立 3D viewer，杆塔形状以来源页等比例预览表达。历史缺陷表保留用于说明 IR 的设计动机。

## 1. 目标与背景

### 1.1 设计时现状缺陷（历史基线）

> **2026-08-24 十样本复核影响**：① 「PHM max depth = 1」约束作废（BIMBase 实测最深 4 层），
> DEV 递归收集必须改为 PHM 递归 + 防环；② 「PHM 矩阵恒 IDENTITY」表述作废，
> GimGeometryInstance 必须携带并应用 PHM 级矩阵；③ stl kind 需区分 binary/ascii；
> ④ xml-mod kind 的 primitive 枚举改开放式；⑤ 新增 .gl 资源类型归入 xml-mod 或单列待定。
...
> 既有 [src/gim/types.ts](../../desktop/src/gim/types.ts) 的 `IfcEntry` 已覆盖此结构，IR 直接复用，不重新发明。
