...
>
> **当前状态（2026-08-30）**：源码以 `PARSER_VERSION=gim-parser-v18`、`GEOMETRY_CACHE_VERSION=geometry-cache-v4-phm-color` 为准；DEV 粒度 GLB 管线、PHM 颜色覆盖、缓存命中回放和属性来源路由已落地。Worker 化解析、SQLite 几何缓存表、InstancedMesh 装配及 `.gimc` 预编译容器仍是长期路线，本文旧版本号仅作演进记录。
...
按 [modAutoLoadService.ts:39-46](../../desktop/src/services/modAutoLoadService.ts#L39-L46) 的常量与 [modAutoLoadService.ts:670-709](../../desktop/src/services/modAutoLoadService.ts#L670-L709) 的批循环：
...
来自 [db.rs:848-941](../../desktop/src-tauri/src/db.rs#L848-L941) 与 [ifcEntryLoader.ts:171-254](../../desktop/src/viewer/ifcEntryLoader.ts#L171-L254)：
...
新增 `geometry_cache` 表（[db.rs](../../desktop/src-tauri/src/db.rs)）：
...
**当前**（[highlight.ts](../../desktop/src/viewer/highlight.ts)）：`raycaster.intersectObjects(scene.children)`，返回 mesh，从 mesh.userData 取 CBM 节点信息。
...
- 当前 [stlLoader.ts](../../desktop/src/viewer/stlLoader.ts) 直接构造 `BufferGeometry`（非 indexed，per-face normal）
...
| 阶段 | 目标 | 改动范围 | 预期收益 |
| ---- | ---- | -------- | -------- |
| **阶段 1**：假并发修复 | 把 `for (const geo of batch)` 改为 `Promise.all(batch.map(...))` | [modAutoLoadService.ts:670-709](../../desktop/src/services/modAutoLoadService.ts#L670-L709) ~20 行 | 4× 吞吐（无新依赖） |
| **阶段 2**：几何缓存表 | 新增 `geometry_cache` 表 + Tauri 命令 | [db.rs](../../desktop/src-tauri/src/db.rs) 新增 ~150 行；[desktop/database.ts](../../desktop/bridge/database.ts) 新增 ~50 行 | 二次打开 < 1 秒 |
| **阶段 3**：Worker 化解析 | 新增 `xmlModParser.worker.ts` + Worker 池 | 新文件 ~300 行；[modAutoLoadService.ts](../../desktop/src/services/modAutoLoadService.ts) 改造 ~100 行 | 主线程不阻塞 |
| **阶段 4**：InstancedMesh | 替换 per-Mesh 为 6 个 InstancedMesh | [xmlModGeometry.ts](../../desktop/src/viewer/xmlModGeometry.ts) 重写 ~400 行；[highlight.ts](../../desktop/src/viewer/highlight.ts) 改造 ~100 行 | draw call 从 108k → 6 |
| **阶段 5**：Material 共享 | Material cache keyed by (r,g,b,a) | [xmlModGeometry.ts:142](../../desktop/src/viewer/xmlModGeometry.ts#L142) ~30 行 | 内存占用 -50% |
...
```text
PARSER_VERSION          = 'gim-parser-v18'        // 当前，GIM 解析层（v1→v18 详见下方演进历史）
...
- [xmlModGeometry.ts](../../desktop/src/viewer/xmlModGeometry.ts) 新增 `_sharedMaterialCache` 按 `(colorHex, opacity, transparent)` 聚类
...
[xmlModGeometry.ts:116-128](../../desktop/src/viewer/xmlModGeometry.ts#L116-L128)：
...
**实现位置**：[xmlModLoader.ts](../../desktop/src/viewer/xmlModLoader.ts) 的 `flattenDocumentToGroup` + [xmlModGeometry.ts](../../desktop/src/viewer/xmlModGeometry.ts) 的 `collectBakedGeometriesByMaterial`。
...
| **阶段 7**：Viewer 加载 .gimc | 检测 .gimc 存在则直接加载 | [modAutoLoadService.ts](../../desktop/src/services/modAutoLoadService.ts) 改造 ~200 行 | 跳过 XML 解析 |
| **阶段 8**：InstancedMesh 装配 | 按 primitiveType 分组，6 个 InstancedMesh | [xmlModGeometry.ts](../../desktop/src/viewer/xmlModGeometry.ts) 重写 ~400 行 | draw call 从 77k → 6 |
...
- [src/services/modAutoLoadService.ts](../../desktop/src/services/modAutoLoadService.ts)：现有加载编排
- [src/viewer/xmlModLoader.ts](../../desktop/src/viewer/xmlModLoader.ts)：MOD 文件 → Group
- [src/viewer/xmlModGeometry.ts](../../desktop/src/viewer/xmlModGeometry.ts)：primitive → BufferGeometry
- [src/viewer/stlLoader.ts](../../desktop/src/viewer/stlLoader.ts)：STL 解析
- [src/gim/geometry/xmlModParser.ts](../../desktop/src/gim/geometry/xmlModParser.ts)：DOMParser-based XML 解析
- [src/viewer/ifcEntryLoader.ts](../../desktop/src/viewer/ifcEntryLoader.ts)：IFC .frag 缓存参考
- [src-tauri/src/db.rs](../../desktop/src-tauri/src/db.rs)：SQLite 表结构与 Tauri 命令
- [src/app/state.ts](../../desktop/src/app/state.ts)：内存缓存字段
- [src/services/projectCleanupService.ts](../../desktop/src/services/projectCleanupService.ts)：项目切换资源释放
