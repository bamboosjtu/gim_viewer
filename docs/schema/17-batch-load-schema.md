...
## 11. 实战教训：从 Material 共享到仍崩溃（2026-07-06 复盘）

> 本节记录 §7.3 阶段 5（Material 共享）落地后的实测结果，以及由此引出的更深层性能瓶颈分析。阶段 5 已实施，但单独不足以解决崩溃，需要叠加 §12/§13/§14 方案。
...
| 指标 | 修复前 | 修复后 |
| ---- | ---- | ---- |
| Material 数量 | ~78000 | 几十个 ✓ |
| MOD 位置正确性 | 横着放 | 直立显示 ✓（叠加 FIX-4 Z-up→Y-up） |
| 加载速度 | 慢 | 提升 ✓ |
| 崩溃临界点 | ~3000 MOD | ~2000+ MOD ✗（仍崩溃） |
...
## 12. 方案 A：Geometry 共享（最小改动，保 Entity 高亮）
...
## 13. 方案 B：mergeGeometries 静态合并（解决 draw call 问题）
...
基于实战教训（§11）与核显预算（§15），更新原 §7 的实施路径：

| 阶段 | 目标 | 改动量 | 收益 | 状态 | 优先级 |
| ---- | ---- | ---- | ---- | ---- | ---- |
| 阶段 1 | 假并发修复（Promise.all） | 小 | 4× 吞吐 | 待评估 | 中 |
| 阶段 5 | Material 共享 | 已实施 | -50% 内存 | ✓ 已完成 | — |
| **阶段 5b** | **Geometry 共享（方案 A）** | **小（~50 行）** | **-40% GPU 内存** | **待实施** | **高** |
| **阶段 5c** | **mergeGeometries 静态合并（方案 B）** | **中（~150 行）** | **draw call 77k → 几十** | **待实施** | **高（核显必需）** |
...
**关键决策**：阶段 5b 实施后若仍崩溃（核显环境很可能），需立即进入阶段 5c（方案 B）。方案 B 牺牲 Entity 级高亮换性能，需用户确认接受。
