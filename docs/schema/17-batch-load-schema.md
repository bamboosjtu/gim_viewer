...
> **注**：本节为初版实施路径，已被 [§16 综合实施路径（更新版）](#16-综合实施路径更新版) 取代。保留作为历史参考。
...
## 11. 实战教训：从 Material 共享到 A.1 解决崩溃（2026-07-06 复盘）

> 本节记录 §7.3 阶段 5（Material 共享）落地后的实测结果，以及由此引出的更深层性能瓶颈分析。
>
> **更新**：方案 A.1（§12）已解决崩溃问题。详见 [18a-experiment-shared-geometry.md](./18a-experiment-shared-geometry.md) §11 实测结论。
...
### 11.4 从"游戏为什么快"得出的修复路径

基于上述对比，修复路径有 3 个层次，按改动量递增、收益递增排序：

| 层次 | 方案 | 改动量 | 收益 | 风险 |
| ---- | ---- | ---- | ---- | ---- |
| 短期 | §12 方案 A：Geometry 共享 | 小（~50 行） | GPU 内存 -40%，draw call 不变 | 低（保 Entity 高亮） |
| 中期 | §13 方案 B：mergeGeometries 静态合并 | 中（~150 行） | draw call 从 77k → 几十，GPU 内存 -60% | 中（Entity 信息丢失） |
| 长期 | §14 方案 C/D：游戏引擎预编译 | 大（架构级） | 加载速度 10×，draw call 6 个 | 高（重写加载管线） |

---

## 12. 方案 A：Geometry 共享（已完成，详见 18a 实验文档）
...
## 13. 方案 B：mergeGeometries 静态合并（暂不需要，保留作为后续优化）

> **状态**：暂不需要实施 — A.1 已解决崩溃问题，7553 MOD 全加载未崩溃。
>
> 保留作为后续优化选项：
> - 若未来需要降低 draw call（当前 ~77000，核显可能帧率较低）
> - 或需要支持更大规模工程（> 10000 MOD）
> - 再考虑实施方案 B（代价：Entity 级高亮降级为 Group 级）

### 13.1 设计

如果方案 A 仍崩溃（draw call 超限主导），需要把同 Material 的多个 geometry 合并成单个 BufferGeometry。
...
**建议**：先实施方案 A 验证。如果仍崩溃，再讨论方案 B（需用户确认是否接受 Entity 高亮降级）。
...
基于实战教训（§11）与 A.1 实测结论（[18a](./18a-experiment-shared-geometry.md)），更新原 §7 的实施路径：
...
| 阶段 5c | mergeGeometries 静态合并（方案 B） | 中 | draw call 77k → 几十 | 暂不需要 | 低（A.1 已解决崩溃） |
| **新问题 1** | **UI 进度显示 bug** | **小** | **进度到 100%** | **待修复** | **高** |
| **新问题 2** | **bbox 跳过 36%（阈值过严）** | **小** | **关键设备不再被跳过** | **待诊断 + 修复** | **高** |
...
**当前优先级**：
1. **高**：修复 UI 进度显示 bug（新问题 1）+ 诊断 bbox 跳过问题（新问题 2）
