...
> 本文只描述当前实现的 DEV/PHM/MOD/GL/STL 几何缓存、批量读取和回退边界。
...
```text
CBM placement → DEV → PHM / 子 DEV → MOD 或 GL 或 STL
```

首次打开由渐进式 DEV GLB 管线编译和渲染；缓存命中优先恢复 DEV 粒度 GLB，只有
单个 DEV 失败时才对该 DEV 做原始 MOD/GL/STL 回退。IFC 仍由 OBC Fragments 独立加载。

## 2. 首次打开流程

```text
可达 CBM/DEV
  → 解析 DEV、PHM、MOD/GL/STL 引用链
...
- `serializeDevToGlb` 在前端完成 MOD/GL/STL 解析、Geometry IR 构造和 GLB 序列化。
...
2. 每个 unique DEV 都有 `glb`、`partial`、`empty` 或 `unsupported` 状态；
3. `glb`/`partial` 文件存在、大小与 manifest 相同且通过 GLB header 校验；
...
```text
manifest status=empty/unsupported → 记录确定性结果，不读文件、不回退
manifest status=glb/partial       → 批量读一次 GLB → 为所有 placement 独立实例化
...
- scoped fallback 只查询失败 DEV 可达的 PHM/MOD/GL/STL，不重新加载成功 GLB DEV。
...
- Phase 4 的 session-local DEV template runtime 对 clean static DEV 共享 immutable
  `BufferGeometry`/base Material，每个 CBM placement 只创建独立 Object3D hierarchy；
  不能证明可共享的 DEV 仍按 DEV 回退 legacy placement。本契约不引入 `InstancedMesh`。
...
- `glb/partial + empty/unsupported` 混合时 fast path 仍算完整命中；合法 empty/unsupported 不触发 raw fallback。
- 缺失、截断、大小错误或 parse exception 只影响失败 DEV；A→B 工程切换后旧批次
  不得提交到 B。
- 变电导航、CBM↔IFC 关系、MOD/GL/STL 来源和属性面板行为与 cache-off 语义一致。
