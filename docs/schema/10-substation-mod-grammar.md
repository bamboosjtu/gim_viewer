...
> **2026-08-24 十样本复核**：14 类 primitive 的封闭枚举被打破。Bentley/BIMBase/SDDP 样本出现
> 至少 13 类新 primitive：Boolean、Cable、Wire、BeamChannel、LightBeamChannel、RotationalEllipsoid、
> Insulator、ConePorcelainBushing、SquareGasket、CircularFixedPlate、BendingCylindrical、
> GimGeCableConcentration（.gl 文件）、H；且 Visible 出现小写 true/false 形态。
> 本文的强类型 schema 结论仅适用于 JinQu 产出；parser 必须改为开放式枚举 + 兜底跳过。
> 各新 primitive 的字段级分析待做。详见 [22](22-ten-sample-verification-0824.md) 与 [08](08-mod-static-survey.md)。
>
> **2026-08-24 StretchedBody / Color 复核**（`desktop/scripts/gim_survey/stretched_color.py`，输出 `_generated/<sid>/stretched-color.csv`）：
>
> | 维度 | 变电站01 (JinQu) | 变电站02 (Bentley) | 变电站03 (BIMBase) | 变电站04 (SDDP) |
> |---|---|---|---|---|
> | Color 总数 | 46250 | 13927 | 50464 | 19118 |
> | A 通道分布 | 100(99.7%)/40 | **0(99.4%)** + 少量 10/30/75/80/**255** | **0(99.9%)**/60 | 100(99.7%)/10 |
> | R/G/B 越界 | 0 | 仅 A=255 共 4 个 | 0 | 0 |
> | StretchedBody 数 | 10263 | 508 | 2353 | 6 |
> | Array 点数 | 3-46 | 3-30 | 3-36 | 4-24 |
> | Normal 长度 | **恒 304.8** | 1.0(64%)+304.8 | **1.0(82%)**+304.8 | 1.0 |
>
> 结论修正：
> 1. 「StretchedBody.Normal 长度恒 304.8（1 英尺）」是 JinQu 特有；Bentley/BIMBase/SDDP 以
>    **单位向量 1.0 为主**、部分保留 304.8。parser 不能对 Normal 长度做归一化假设之外的写死。
> 2. 「A 取值 40 或 100」被推翻：Bentley/BIMBase 的 A 以 **0 为绝对主导**，且出现 10/30/60/75/80/255。
>    **实现风险**：若渲染端把 A 当透明度直接使用，A=0 会把整批设备渲染成全透明；
>    A 的语义（透明度 vs 不透明度 vs 未启用）必须先在渲染层验证，不能沿用旧假设。
"> 3. R/G/B ∈ [0,255] 在 4 样本全部成立；RGB 主色 (138,149,151) 跨工具一致（设备灰）。
> 4. **A=0 语义判定（P0-2 结论）**：按 primitive × A 值交叉统计，A=0 均匀覆盖全部实体类型
>    （Cylinder/Cuboid/Boolean 等实体设备同样 A=0）——若 A 是透明度则整批设备不可见，与工程事实矛盾。
>    **判定：Bentley/BIMBase 的 A=0 是"不透明默认值"而非全透明。** 渲染策略应为
>    `alpha = (A > 0) ? normalize(A) : 1.0`。零星非零 A（Sphere A=30、Cuboid A=60/100 等）
>    才是真实透明意图候选。
> 5. **A 刻度判定规则（P3-2 结论，四工具全量实测）**：
>    | 工具 | A 值域 | 刻度 |
>    |---|---|---|
>    | JinQu | {40, 100} | 百分制 |
>    | SDDP | {10, 100} | 百分制 |
>    | BIMBase | {0, 60} | 百分制 |
>    | Bentley | {0,10,30,75,80,100,**255**} | **字节制** |
>
>    判定规则：`max(A) > 100 → 除以 255，否则除以 100`（按文件级扫描判定）。
>    不透明哨兵分两族：JinQu/SDDP 用 `A=max`（100→不透明），Bentley/BIMBase 用 `A=0`（0→不透明）。
>    统一渲染伪码：
>    `opaqueIsZero = (A=0 占比 > 50%)`；
>    `alpha = opaqueIsZero ? (A>0 ? A/scale : 1) : A/scale`。
...
| 脚本 | 路径 | 用途 |
| ---- | ---- | ---- |
| xml-primitive-survey.ps1 | [skill scripts/xml-primitive-survey.ps1](../../.agents/skills/gim-sample-verification/scripts/xml-primitive-survey.ps1) | Round 6 主分析：扫描全部 MOD XML，输出 primitive-attrs.csv（每行一个属性）与 primitive-summary.csv（按 primitive×field 聚合统计） |
| color-analysis.ps1 | [skill scripts/color-analysis.ps1](../../.agents/skills/gim-sample-verification/scripts/color-analysis.ps1) | Color 节点单独分析：扫描 R/G/B/A 4 个独立属性，输出分布表与 Top 20 组合 |
| stretched-body-deep.ps1 | [skill scripts/stretched-body-deep.ps1](../../.agents/skills/gim-sample-verification/scripts/stretched-body-deep.ps1) | StretchedBody 深度分析：解析 Array 点序列与 Normal 向量，输出点数分布、坐标全局范围、Normal 长度分布 |
...
powershell -NoProfile -ExecutionPolicy Bypass -File `
  ...
  -OutDir ".agents/skills/gim-sample-verification/scripts/demo-substation"
