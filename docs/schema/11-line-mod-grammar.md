...
>
> **2026-08-24 六线路样本复核**（证据：`_generated/line-grammar-summary.csv`，脚本 `desktop/scripts/gim_survey/line_grammar.py`）：
>
> **保持成立**：4 类格式族在全部 6 个线路样本零逃逸（无新格式族）；POINT 值恒 5 token、LINE 恒 2 token、type 仅 {13,42}；G 记录恒 6 token；R 记录 11-token 主导；HNUM 层级结构 H→BodyN→HBodyN/P/R/G/SubLegN 复现。
>
> **需修正/扩展**：
> 1. **BoltN 首段实为 12 个逗号 token**（非旧记录的 15）；分号段数有两种形态：
>    `{首段12token, 尾段4token}`（2 段，demo-line1/线路02）与
>    `{首段12token, 4个单token段, 尾段4token}`（6 段，线路04/线路05/线路06）。parser 不能写死段数。
> 2. **BoltNum 出现新值 12**（线路05，12 个文件），旧基线“仅 4 或 8”作废，应按 `BoltNum=N` 动态读取。
> 3. **CODE 新取值大量出现**：`1019`（线路02 290/线路03 221/线路05 69）、`191`（线路05 216）、
>    `81/82`（demo-line1）、`37/404/523`。“CODE 与 CROSS 类型的静态映射”进一步被否定。
> 4. **TEXT_KEY_VALUE 签名从 2 种扩展到至少 5 种**：
>    - `type,H1,H2,H3,H4,d,D,e1,e2`（含 D，最常见）
>    - `type,H1,H2,H3,H4,d,D`（无 e1/e2，线路03）
>    - `type,H1,H2,H3,d,D,α1,α2,e1,e2`（希腊字母字段，线路02 20 文件）
>    - `type,H1..H6,d,D,b,B1,L1,e1,e2,cs,ZCOUNT,ZPOSTARRAY`（超长塔型签名，线路02/线路05各 1）
>    - `TYPE,SECTIONALAREA,...`（WIRE 导线参数，全样本一致）
>    **parser 的 D 可选、弱 schema 兑底策略得到多样本支持。**
> 5. 部分 HNUM/KV 文件带 UTF-8 BOM，解析必须用 utf-8-sig。
>
> **2026-08-24 P1 补充：CODE 业务映射反推 + 新 KV 签名字段语义**
> （CROSS→DEV→PHM→MOD 链关联 FAM 跨越物属性实证，6 线路样本聚合）：
>
> **CODE 映射表（首个跨样本业务语义证据）**：
>
> | CODE | 被跨越物类型 | FAM 佐证字段 |
> |---|---|---|
> | 201 | **房屋**（957 例） | 房屋高度/屋顶类型/层数/房屋面积/房屋种类/权属 |
> | 191 | **河流水域**（216 例） | 宽度/水位高/是否通航/50年通航桅杆高/100年洪水位 |
> | 1019 | **树木**（586 例） | 树高/树木种类(杂树)/密度 |
> | 30~37 | **电力线**（按电压等级细分：30=AC220V、31=AC380V 等） | 名称=电力线/电压等级/高温·低温·年平均·大风K值 |
> | 523 | 地下通信电缆 | 等级=地下通信电缆 |
> | 81/82/404 | 少量未定（FAM 无名称字段） | - |
>
> **parser 边界更新**：CODE 与被跨越物类型的对应在跨样本上稳定，
> 可作为展示层语义标注使用；但电压等级细分（30~37 各档含义）仍需更多样本确认。
>
> **新 KV 签名定性修正**：含 `α1/α2` 的签名（`type,H1,H2,H3,d,D,α1,α2,e1,e2`）与含
> `ZCOUNT/ZPOSTARRAY` 的超长签名的 `type` 值分别为「掏挖基础」「承台灌注桩基础」——
> 这些文件是**杆塔基础参数表**而非塔身参数：
> - `H1..H6`=分段高度，`d`=主径，`D`=扩底径，`e1/e2`=偏心，`α1/α2`=放坡角
> - `ZCOUNT`=桩数，`ZPOSTARRAY`=`(x,y,z)` 桩位坐标序列（实测 4 桩承台）
> 因此 TEXT_KEY_VALUE 的设备归属应为「WIRE 导线参数 + BASE 基础参数」两大类，
> 基础参数可支撑基础的参数化三维重建。
>
> **历史复核（2026-07-17）**：重新扫描 `demo-line` 与 `demo-line1`，四类格式族数量与 grammar 结论保持一致。`src/gim/geometry/lineModParser.ts` 已实现四类 parser，38 个测试已通过；“仅测试引用、运行时尚未消费”是当时的状态快照。
>
> **当前状态（2026-08-30）**：parser 已由 `desktop/src/services/lineModRuntimeService.ts` 接入线路属性面板。HNum、Bolt、Tower_Device、WIRE 和点线格式在参数/来源页按需展示；线路工程仍只提供“模型”地图工作区，不启用独立线路 3D。
>
...
### 8.1 历史运行时缺口（2026-07-17 基线）

在 2026-07-17 的快照中，线路 MOD 的语法解析已实现，但 `parseLineMod` 只被单元测试调用，尚未接入运行时加载/渲染，导致：
...
> **历史范围与实现更新（2026-07-17）**：原项目硬约束"MVP 不实现悬链线、3D 线路、MOD 解析"已调整。线路 MOD parser 已基于本报告落地；运行时消费与线路几何渲染当时仍未落地。当前 parser 已接入属性面板，独立线路 3D 仍不在产品边界内。

- ~~项目硬约束："MVP 不实现悬链线、3D 线路、MOD 解析"~~ → **已调整：线路 MOD parser 纳入 MVP 范围**
- ~~本报告仅形成 parser 草案边界，不进入渲染实现~~ → **Parser 已实施；属性面板运行时消费已接入，独立线路 3D 仍不实施**
...
| 脚本 | 路径 | 用途 |
| ---- | ---- | ---- |
| line-mod-grammar-deep.ps1 | [skill scripts/line-mod-grammar-deep.ps1](../../.agents/skills/gim-sample-verification/scripts/line-mod-grammar-deep.ps1) | Round 7 主分析：扫描 4 类文本格式族，输出 HNum/POINT_LINE/Section_KV/Key_Value 4 个 CSV 与文本报告 |
...
powershell -NoProfile -ExecutionPolicy Bypass -File `
  ...
  -OutDir ".agents/skills/gim-sample-verification/scripts/demo-line"
```
