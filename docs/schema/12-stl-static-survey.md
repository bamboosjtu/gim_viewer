...
> - 线路 6 样本：STL 与 MOD 在 PHM 级互斥不变；STL 设备类型仍为金具/绝缘子串
>   （SYMBOLNAME=INSULATOR/FITTINGS/SPACER/DAMPER），不渲染塔本体。
> - 三角面统计量级与旧基线一致。
>
> **需修正**：
> 1. **「100% binary」作废**：line03 出现 3 个 ASCII STL（header `solid AssimpScene`，
>    非 SLW3D 工具）。STL loader 必须兼容 ASCII 分支。
> 2. **「变电 STL = 二次柜专属」作废**：substation03（BIMBase）的 STL 渲染
>    **一次设备**（TYPE top: LightningProtector/EarthingSwitch/FrameCapacitorBank；
>    SYMBOLNAME 如 110kV氧化锌避雷器/220kV接地开关/10kV电容器组）。
>    变电 STL 的设备归属由导出软件决定，不是格式规则。
> 3. 变电站02（Bentley）无任何 STL——STL 是可选几何载体。

> **2026-08-24 R8.5/R8.6 六维度全量聚合**（`desktop/scripts/gim_survey/device_type_survey.py`，
> 输出 `_generated/device-type-survey-<sid>.json`）：
>
> - **线路 6/6 样本映射完全复现**：STL=INSULATOR/FITTINGS/SPACER/DAMPER（金具）；
>   CROSS→TEXT_POINT_LINE(100%)、WIRE→TEXT_KEY_VALUE(100%)、WIRE_DEVICE→STL-only(100%)、
>   TOWER_DEVICE→HNUM+SECTION_KV+KEY_VALUE+塔下 STL 挂载。
> - **变电 STL/MOD 分工随工具分化**：
>   | 样本 | STL 角色 | MOD 角色 |
>   |---|---|---|
>   | 变电站01 (JinQu) | 二次柜 SecondaryCabinet | 一次设备 primitive |
>   | 变电站02 (Bentley) | **无 STL** | 一次+二次全走 XML primitive |
>   | 变电站03 (BIMBase) | **一次设备**（电容器组/避雷器/接地开关） | HGIS/互感器等 XML |
>   | 变电站04 (SDDP) | 少量一次设备（消弧线圈成套装置） | 屏柜/线夹/绞线 XML |
