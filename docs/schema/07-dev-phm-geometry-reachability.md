...
目标：分析几何资源如何从 CBM 实例一路走到 MOD/STL。

> **2026-08-24 P1-4：`.gl` 引用机制已查明**。45 个 .gl 全部与设备级 `.cbm`/`.dev` 同 UUID
> （110kV隔离开关、220kV支柱绝缘子、电容式电压互感器等真实设备），内容为
> `GimGeCableConcentration`（电缆集中布置连线，含 ConnectGuid/ConnectionRules）。
> 它们**设计上不进入 SOLIDMODELn 渲染链**，而是与设备节点同 UUID 隐式绑定的旁路辅助几何；
> 若需可视化电缆集中布置，应按「同 UUID 绑定」路径加载，而非修改引用遍历。
>

> **2026-08-24 十样本复核**（证据：`_generated/<sid>/reachability.csv`，脚本 `desktop/scripts/gim_survey/mod_reachability.py`）：
>
> | 样本 | MOD 可达/总数 | STL 可达/总数 | GL 可达/总数 | PHM 嵌套深度 | 孤儿特征 |
> |---|---|---|---|---|---|
> | demo-substation | 4135/4179 | 1803/1803 | - | 0 | 44 个 EMPTY_DEVICE_XML（与旧基线一致） |
> | substation02 | 627/627 | 无 STL | - | 0 | 无 |
> | substation03 | 2570/2570 | 556/556 | **0/45** | **4** | 45 个 .gl 全部孤儿 |
> | substation04 | **228/703** | 1/1 | - | 0 | **475 个 XML MOD 孤儿** |
> | demo-line1 | 360/508 | 82/82 | - | 0 | 148 个 TEXT_POINT_LINE 孤儿（与旧基线一致） |
> | 其余 4 线路样本 | 全部可达 | 全部可达 | - | 0 | 无 |
>
> 结论：
> 1. 「可达率接近但不必等于 100%」在 10 样本成立；但孤儿规模差异极大（0 到 475），且变电站04样本的
>    孤儿是带 Entity 的 XML MOD，不再是旧基线的 EMPTY_DEVICE_XML 单一成因。**不可达 MOD 不代表空几何。**
> 2. PHM 嵌套（substation03 深度 4）要求可达性遍历必须递归 PHM 并防环；
>    旧「DEV 最大深度 = 1、PHM 不嵌套」的约束作废。
> 3. entityName → 几何 kind 映射在全部新样本复现：CROSS→TEXT_POINT_LINE(100%)、
>    WIRE→TEXT_KEY_VALUE(100%)、WIRE_DEVICE→STL(100%)、TOWER_DEVICE→HNUM+SECTION_KV+KEY_VALUE(+STL)、
>    变电 F4System/PARTINDEX→XML(+少量 STL)。
