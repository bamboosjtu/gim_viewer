...
> **2026-08-24 十样本复核**（证据：`_generated/<sid>/mod-kind.csv` 与 `primitive-survey.csv`，脚本 `desktop/scripts/gim_survey/mod_reachability.py`）：
>
> **保持成立**：线路 6 样本的 MOD 全部落在已知 4 类文本格式族内，无新格式族；demo-substation 分布与旧基线完全一致（4135 XML_WITH_ENTITIES + 44 EMPTY_DEVICE_XML）；JinQu XML 带 UTF-8 BOM（解析必须用 utf-8-sig）。
>
> **重大扩展**：Bentley/BIMBase/SDDP 三个新变电工具的 MOD 虽同为 `<Device><Entities><Entity>` XML，但 primitive 家族大幅超出旧基线的 14 类，新增至少 13 类：
>
> | 新 primitive | 出现样本（Entity 数） |
> |---|---|
> | `Boolean` | 变电站02 2820 / 变电站03 6694 |
> | `Cable` | 变电站02 97 |
> | `Wire` | 变电站02 67 / 变电站03 12 / 变电站04 104 |
> | `BeamChannel` | 变电站02 6 / 变电站03 58 |
> | `LightBeamChannel` | 变电站03 22 |
> | `RotationalEllipsoid` | 变电站02 2 / 变电站03 18 / 变电站04 4 |
> | `Insulator` | 变电站02 1 |
> | `ConePorcelainBushing` | 变电站04 6 |
> | `SquareGasket` / `CircularFixedPlate` | 变电站04 各4 |
> | `BendingCylindrical` | 变电站04 2 |
> | `GimGeCableConcentration` | 变电站03 .gl 文件 201 |
> | `H` | 变电站03 4 |
>
> 同时 `Visible` 属性出现小写形态 `true/false`（Bentley/BIMBase），旧基线 `True/False` 仅 JinQu/SDDP。
>
> **2026-08-24 P0-3：新 primitive 字段级分析 + 渲染缺件量化**
> （`desktop/scripts/gim_survey/primitive_gap.py`，输出 `_generated/<sid>/primitive-fields.csv`）：
>
> **缺件量化**（渲染白名单 = 7 类：Cylinder/Cuboid/Sphere/TruncatedCone/Ring/CircularGasket/StretchedBody）：
>
> | 样本 | Entity 总数 | 可渲染 | 缺件率 | 最大缺口 |
> |---|---:|---:|---:|---|
> | 变电站01 (JinQu) | 46250 | 95.7% | 4.3% | PorcelainBushing 1506 |
> | 变电站02 (Bentley) | 13647 | 74.1% | **25.9%** | **Boolean 2820** |
> | 变电站03 (BIMBase) | 50464 | 83.6% | 16.4% | **Boolean 6694** |
> | 变电站04 (SDDP) | 19118 | 94.0% | 6.0% | OffsetRectangularTable 681 |
>
> 缺件集中设备：绝缘子/电容器/隔离开关（JinQu）、各类屏柜（Bentley 的柜体大量用 Boolean 开孔）、
> 套管/电抗器（BIMBase）、主变套管与绞线（SDDP）。
>
> **新类型字段语义要点**：
> - `Boolean`：`Type`(字符串，布尔运算类型) + `Entity1`/`Entity2`(同 MOD 内 Entity ID 引用，
>   数值范围到 5 万)。**无法独立参数化重建——必须实现同文件内 Entity 间 CSG 或引用组合**，
>   这是 Bentley/BIMBase 缺件的主因。
> - `Wire`（区别于线路 MOD 文本格式）：`D`(直径)、`Sag`(弧垂) +
>   `StartCoord/EndCoord/StartVector/EndVector/FitCoordArray`（拟合点序列）
>   ——变电软母线的完整弧垂数据已内嵌，可直接按样条渲染。
> - `Cable` / `GimGeCableConcentration`：直径 + `InflectionCoordArray` 拐点序列（电缆敷设路径）。
> - `BeamChannel`/`LightBeamChannel`/`H`：`Length` + `Model` 型号字符串（槽钢截面需型号查表）。
> - `ConePorcelainBushing`：PorcelainBushing 的锥形变体（BR/TR 双半径组 + N 伞裙数）。
> - 其余数值字段全部为正（无负值/零值异常），单位延续 mm。
>
> **2026-08-24 P3-1：Boolean CSG 使用模式**（变电站02/03 全量）：
> - 运算类型分布：`Difference` 7405（77%）、`Union` 1868（19%）、`Intersection` 241（2.5%）。
> - 引用范围：9514/9514 全部指向**本文件内** Entity，无悬空引用。
> - **链式嵌套率 49%**（4574 条 Boolean 的被引对象仍是 Boolean）→ 构成 CSG 树，
>   实现必须按依赖拓扑排序求值。
> - 实现策略建议：加载期用 three-bvh-csg 按 SUBTRACTION/UNION/INTERSECTION
>   拓扑序重建，结果按 MOD 缓存；或离线预计算网格。文件数少（70/151 个 MOD 含 Boolean），
>   可懒加载处理。
**变电 XML parser 的 primitive 分发必须是开放式枚举 + 兜底跳过，不能封闭在 14 类。**

> **2026-07-17 复核**：修正 `_generated/mod-static-profile-v2.ps1` 对 UTF-8 BOM 的处理后重新扫描三组样本，MOD 总数及四类线路文本格式族、变电 `4135 XML_WITH_ENTITIES + 44 XML_EMPTY_DEVICE` 的分布均与本文一致。本文的“不进入渲染实现”描述的是研究范围，不代表当前产品仍未实现 MOD parser/渲染。
