...
> **2026-08-24 指针**：PHM 矩阵恒 IDENTITY 的前提已被十样本复核推翻（BIMBase/SDDP 含非单位阵），
> 最新结论见 [22-ten-sample-verification-0824.md](22-ten-sample-verification-0824.md) 与 [09](09-transform-chain-analysis.md) 复核段。
...
`applyExternalTransforms` 函数原先存在于 [xmlModLoader.ts](../../desktop/src/viewer/xmlModLoader.ts)，直接 `group.applyMatrix4` 不缩放平移，且参数仅含 DEV/PHM 缺少 CBM/SUBDEVICE 累积。
...
[xmlModGeometry.ts:116-148](../../desktop/src/viewer/xmlModGeometry.ts#L116-L148)（修复前）：
...
[modAutoLoadService.ts:53-66](../../desktop/src/services/modAutoLoadService.ts#L53-L66) 中 `state.loadedXmlModGroups` 是 `Map<instanceKey, Group>`，**无上限、无 LRU 驱逐策略**，所有 Group 永久驻留。
...
2. **新增 `disposeSharedXmlModMaterials`**：统一释放共享缓存，由 [projectCleanupService.ts](../../desktop/src/services/projectCleanupService.ts) 在所有 MOD Group 从 scene 移除后调用。
...
3. [modAutoLoadService.ts:52-66](../../desktop/src/services/modAutoLoadService.ts#L52-L66) 创建的 `modRootGroup` 直接挂在 scene 下，**没有任何坐标轴转换**
4. 每个 MOD Group 的变换链：
   ```
...
| 编号 | 问题 | 修复位置 | 状态 | 验证方法 |
| ---- | ---- | -------- | ---- | -------- |
| FIX-1 | modPath 去重 bug | [modAutoLoadService.ts](../../desktop/src/services/modAutoLoadService.ts) | ✅ 已完成 | 改用 instanceKey 去重；多实例 MOD 文件位置正确填充；"MOD 完全覆盖 IFC"现象消失 |
| FIX-2 | applyExternalTransforms 隐患 | [xmlModLoader.ts](../../desktop/src/viewer/xmlModLoader.ts) | ✅ 已删除 | 函数已从源码删除；生产路径全部走 applyPlacementTransformToSceneUnits（顶点烘焙版） |
| FIX-3 | 每 Entity 独立 Material 导致 OOM 崩溃 | [xmlModGeometry.ts](../../desktop/src/viewer/xmlModGeometry.ts) + [projectCleanupService.ts](../../desktop/src/services/projectCleanupService.ts) | ✅ 已完成 | 加载 7000+ MOD 不崩溃；Material 数从 78000+ 降到几十；项目切换时共享 Material 正确释放 |
| FIX-4 | GIM Z-up 与 viewer Y-up 坐标系不一致 | [coordinateAlignmentService.ts](../../desktop/src/services/coordinateAlignmentService.ts) `syncProjectSourceToViewerFromFragments` + `makeTranslationAlignment` | ✅ 已完成 | 二次屏柜直立显示（屏柜"高度"方向映射到 viewer +Y）；CBM 矩阵 Tz=5750mm 旋转后映射到 viewer +Y 约 5.75 米 |
