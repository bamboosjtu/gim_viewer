...
- **JSON 内容**：`generatedAt` + `parserVersion`（可选）+ `projectSummary` + 完整 `LineCatenaryParamAuditReport`（含覆盖率统计 + 4 类样本，每类最多 20 条）+ `spanGroupingReport`（M4-B3B 档距聚合：唯一档距数 / 每档 WIRE 数 min/max/avg / MATRIX0 平移分量 x/y/z 范围 / KVALUE=0 分布，最多 20 档距样本，每档最多 20 条 WIRE 样本）。
- **Markdown 摘要章节**：§1-§9（M4-B3 覆盖率 / KVALUE / SPLIT / MATRIX0 / BLHA / 语义假设 / 阻塞 / 建议）+ §10 档距聚合摘要（WIRE 总数 / 唯一档距数 / min/max/avg / Top 5）+ §11 MATRIX0 平移样本（前 5 档距的 wireTypes / SPLIT / P0/P1 zRange / KVALUE 0/非0 占比 + 观察 + 阻塞 + 建议）。
...
- **文档**：详见 [docs/m4-b3-catenary-param-validation.md](m4-b3-catenary-param-validation.md) §9 用户核验流程 + [docs/m4-b3b-span-grouping-validation.md](m4-b3b-span-grouping-validation.md) 档距聚合核验要点。
