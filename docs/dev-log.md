...
- **JSON 内容**：`generatedAt` + `parserVersion`（可选）+ `projectSummary` + 完整 `LineCatenaryParamAuditReport`（含覆盖率统计 + 4 类样本，每类最多 20 条）+ `spanGroupingReport`（M4-B3B/B3C 档距聚合 + 拓扑分类：唯一档距数 / 每档 WIRE 数 min/max/avg / MATRIX0 平移分量 x/y/z 范围 / KVALUE=0 分布 / groupKindCounts / samePointGroupSamples / interPointSpanSamples / missingEndpointGroupSamples，最多 20 档距样本 + 每类 10 样本，每档最多 20 条 WIRE 样本）。
- **Markdown 摘要章节**：§1-§9（M4-B3 覆盖率 / KVALUE / SPLIT / MATRIX0 / BLHA / 语义假设 / 阻塞 / 建议）+ §10 档距聚合摘要 + WIRE 拓扑分类（M4-B3C：same-point / inter-point / missing-endpoint group 数与 WIRE 数表）+ §11 inter-point 跨点档距样本（前 5 档距的 distance / wireTypes / SPLIT / P0/P1 zRange / KVALUE 0/非0 占比 + 观察 + 阻塞 + M4-B3C 收口建议）。
...
- **M4-B3C 收口**：M4-B3C 后，悬链线不进入当前 MVP 实现范围。当前线路地图继续使用直线段。Ctrl+Shift+C 仅作为审计和后续研究工具。
- **文档**：详见 [schema/14-line-catenary-study.md](schema/14-line-catenary-study.md) §5 用户核验流程 + §3.5 WIRE 拓扑分类 + §6 决策与后续路线。
...
> M4-B3 / B3A / B3B / B3C 审计已完成，但以下项**暂缓或待进一步研究**。
>
> 已证实的结论见 [gim_line.md](gim_line.md) §11 WIRE 拓扑分类与悬链线候选字段。
>
> 决策记录与研究方法论见 [schema/14-line-catenary-study.md](schema/14-line-catenary-study.md)。
>
> 审计工具：`Ctrl+Shift+C` 导出 JSON + Markdown 摘要（详见 [schema/14-line-catenary-study.md](schema/14-line-catenary-study.md) §4 审计服务 API + §5 用户核验流程）。

### 8.1 KVALUE 物理含义（暂缓）

- **现状**：KVALUE 为数值类型，覆盖率 100%，但物理含义未确认
- **候选假设**：张力系数 / 弧垂参数 / 应力 / 单位无关参数
- **阻塞原因**：GIM 标准文档无明确字段定义
- **核验方式**：用户对照样本工程，按 inter-point span 的 KVALUE 分布核验
- **影响**：未确认前不进入悬链线实现

### 8.2 MATRIX0 平移分量语义（暂缓）

- **现状**：格式已确认（16 元素 4x4 矩阵），平移分量在 `values[12]/[13]/[14]`
- **待确认**：
  - 单位（米？毫米？）
  - 坐标系（局部？世界？相对塔位？）
  - x/y 是否为横担偏移
  - z 是否为挂点高度层级
- **核验方式**：用户对照样本工程，对比 CONDUCTOR vs OPGW 的 zRange 是否分层
- **影响**：未确认前不作为挂点坐标使用

### 8.3 BLHA 是塔位还是挂点（暂缓）

- **现状**：BLHA 格式已确认（`纬度,经度,高程,方位角`），但代表塔位中心还是挂点位置未确认
- **关键观察**：same-point group 中 POINT0.BLHA == POINT1.BLHA，若 BLHA 为塔位中心则说明同塔内部连接
- **核验方式**：用户对照样本工程，对比 BLHA 高程与塔位 FAM towerHeight
- **影响**：影响 MATRIX0 平移分量的解释

### 8.4 same-point vs inter-point 的 KVALUE / zRange 差异（暂缓）

- **现状**：M4-B3C 已分离 same-point 与 inter-point，但差异分析需用户核验
- **待确认**：
  - same-point 中 KVALUE=0 是否占多数（疑似同点内部连接未启用弧垂参数）
  - inter-point 中 KVALUE 非 0 是否占多数（疑似弧垂/张力候选参数）
  - MATRIX0 zRange 在 same-point / inter-point 中是否表现不同
- **核验方式**：`Ctrl+Shift+C` 导出 JSON，查看 `spanGroupingReport.observations`

### 8.5 悬链线实现（后置到 M5）

- **决策**：M4 不实现悬链线，地图保持直线段显示
- **后续路线**（详见 [schema/14-line-catenary-study.md](schema/14-line-catenary-study.md) §6.5 后续可能路线）：
  - M5-A：真实跨点档距识别（基于 inter-point）
  - M5-B：MATRIX0 挂点坐标确认
  - M5-C：KVALUE 物理含义确认
  - M5-D：示意悬链线 feature flag
  - M5-E：工程语义悬链线
- **进入 M5 前置条件**：用户对照样本工程核验上述 8.1-8.4 全部完成
